...
namespace Noris.Clients.Win.Components.AsolDX
{
    ...
    public class MsWebView : Microsoft.Web.WebView2.WinForms.WebView2
    {
        ...
        private void _CoreWebView2InitializationCompleted(object sender, Microsoft.Web.WebView2.Core.CoreWebView2InitializationCompletedEventArgs e)
        {
            ...
            {
                // K této chybě dojde mimo try  catch blok (=> asynchronně, invokováno do GUI), takže throw mi shodí aplikaci. Dáme placku:
                DxComponent.ShowMessageException(e.InitializationException, "", "MsWebView error");
        ...
        {
            var webProperties = this.WebProperties;
            if (webProperties is null) return;                       // Nejspíš událost _DoNavigationTotalCompleted doběhla až po Dispose objektu. To může být, když _DoNavigationTotalCompleted je řízena časovačem, resetovaným událostí WebRequestDone.

            switch (webProperties.DisplayMode)
            {   // Pokud jsem v režimu Capture... :
                case DxWebViewDisplayMode.CaptureAsync:
