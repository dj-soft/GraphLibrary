...
White
    ...
    public static class Convertor
    {
        ...
        */
        ...
        #region XML entity
        /// <summary>
        /// Konvertuje čitelný text do XML contentu.
        /// Řeší náhradu XML entit za stringové znaky.
        /// </summary>
        /// <param name="text"></param>
        /// <param name="isAttribute"></param>
        /// <param name="replaceWithChar">Pokud bude zadán znak, bude použit namísto znaků které by byly nahrazeny entitou. Vhodné pro tvorbu popisných textů, nikoli XML.</param>
        /// <returns></returns>
        /// <exception cref="InvalidOperationException"></exception>
        public static string StringToXmlText(string text, bool isAttribute = false, char? replaceWithChar = null)
        {
            if (text is null) return "";

            var sb = new StringBuilder(text.Length);
            bool doReplace = (replaceWithChar.HasValue);
            string replaceText = (doReplace ? replaceWithChar.Value.ToString() : "");
            foreach (var chr in text)
            {
                if (chr == '<')
                    sb.Append(doReplace ? replaceText : "&lt;");
                else if (chr == '>')
                    sb.Append(doReplace ? replaceText : "&gt;");
                else if (chr == '&')
                    sb.Append(doReplace ? replaceText : "&amp;");
                ...
                // Legal sub-chr32 characters
                    ...
                    sb.Append(isAttribute ? (doReplace ? replaceText : "&#xA;") : "\n");
                else if (chr == '\r')
                    sb.Append(isAttribute ? (doReplace ? replaceText : "&#xD;") : "\r");
                else if (chr == '\t')
                    sb.Append(isAttribute ? (doReplace ? replaceText : "&#x9;") : "\t");
        ...
        /// <summary>
        /// Konvertuje XML content do čitelného textu.
        /// Řeší náhradu XML entit za stringové znaky.
        /// </summary>
        ...
        public static string XmlTextToString(string xmlText)
        {
            ...
            int length = xmlText.Length;
            int pointer = 0;
            while (pointer < length)
            {
                string part = searchPart(out bool isEntity);
                if (isEntity)
                {   // result v případě isEntity obsahuje jen důležité znaky, nikoliv úvodní  &    a koncový  ;
                    string entity = part.ToLower();
                    switch (entity)
                    {
                        ...
                        case "#xa": sb.Append("\n"); break;
                        case "#xd": sb.Append("\r"); break;
                        case "#x9": sb.Append("\t"); break;
            ...
            string searchPart(out bool isEnt)
            {
                string result;
                int nextApost = xmlText.IndexOf("&", pointer);
                if (nextApost < 0)
                {   // 1. Od indexu 'pointer' už nemáme žádný apostrof = vše je prostý text:
                    result = xmlText.Substring(pointer);
                    pointer = length;
                    isEnt = false;
                }
                else if (nextApost == pointer)
                {   // 2. Právě na indexu 'pointer' je apostrof = budeme jej zpracovávat nyní:
                    int endEntity = xmlText.IndexOf(";", nextApost);
                    if (endEntity > nextApost)
                    {   // Na pozici 'pointer' je &, pak je něco (=hledaná entita = result), a na pozici endEntity je znak ;
                        result = xmlText.Substring(pointer + 1, endEntity - pointer - 1);
                        pointer = endEntity + 1;           // Příští hledání začne až za znakem ;  ten do resultu nejde
                        isEnt = true;
                    }
                    else
                    {   // Chybný formát, protože za & následuje entita a končí ;
                        result = "";
                        pointer++;
                        isEnt = false;
                    }
                ...
                {   // 3. Na indexu 'pointer' začíná běžný text, např. ABCDE, ale pak na indexu 'nextApost' je & a za ním bude XML entita.
                    //    Nyní zpracujeme text ABCDE, a 'pointer' nastavíme na 'nextApost', tím v příštím kroku zpracujeme XML entitu zdejšíé větví "2"
                    result = xmlText.Substring(pointer, nextApost - pointer);
                    pointer = nextApost;
                    isEnt = false;
                }
                return result;
