...
";
    ...
    internal class Parser : IDisposable
    {
        ...
        /// <summary>
        /// Vygeneruje a vrátí new parser pro zadaná pravidla
        /// </summary>
        /// <param name="setting"></param>
        /// <returns></returns>
        public static Parser CreateParser(Setting setting) { return new Parser(setting); }
        /// <summary>
        ...
        {
            return "Parser: " + this.SettingName + "; Segments: " + this._SegmentSettings.Count.ToString();
        }
        ...
        private void _AnalyseSetting()
        {
            ...
            // Do dictionary nasypu analyzované settingy (SettingSegmentCompiled) jednotlivých vstupních segmentů (SettingSegment):
        ...
        internal class SettingSegmentCompiled
        {
            ...
            internal SettingSegmentCompiled(SettingSegment segmentSetting)
            {
                ...
                this.PermittedFamilies = segmentSetting.PermittedFamilies;
                this.HasPermittedFamilies = segmentSetting.PermittedFamilies.HasValue;
            ...
            /// <summary>
            /// true pokud je zadána hodnota <see cref="PermittedFamilies"/>
            /// </summary>
            internal bool HasPermittedFamilies { get; set; }
            /// <summary>
            /// Povolené rodiny znaků.
            /// Pokud je zadáno, pak znaky v tomto segmentu musí být z některé skupiny znaků, které jsou v této proměnné uvedeny.
            /// Znak se vyhodnocuje typicky pomocí metod typu: <see cref="Char.IsDigit(char)"/> pro <see cref="CharacterFamilyType.Digit"/>.
            /// Pokud je null, nevyhodnocuje se.
            /// Souběh zadání <see cref="PermittedFamilies"/> a <see cref="PermittedsChar"/> je operátorem OR, 
            /// tzn. pokud znak vyhovuje alespoň z hlediska Family, anebo je uveden v povolených znacích, pak je akceptován jako povolený znak segmentu.
            /// Lze tak povolit všechny číslice a písmena + přidat znak "_".
            /// </summary>
            internal CharacterFamilyType? PermittedFamilies { get; set; }
            /// <summary>
            /// true pokud obsahuje něco v <see cref="PermittedsChar"/>
            /// </summary>
            ...
            /// <returns></returns>
            internal SegmentCurrentCharType DetectChange(char[] input, ref int pointer, out string text, out SettingSegmentCompiled nextSetting)
            {
                nextSetting = null;
                text = null;
                ...
                if (this.IsStopOfThisSegment(input, ref pointer, out text))
                    return SegmentCurrentCharType.StopCurrentSegment;
                ...
                if (this.IsBeginOfInnerSegment(input, ref pointer, out text, out nextSetting))
                    return SegmentCurrentCharType.BeginNewSegment;

                // Detekujeme Blank:
                if (this.IsBlank(input, ref pointer, out text))
                    return SegmentCurrentCharType.Blank;

                // Detekujeme Delimiter:
                if (this.IsDelimiter(input, ref pointer, out text))
                    return SegmentCurrentCharType.Delimiter;

                // Zjistíme, zda vstupující text je/není ilegální:
                if (this.IsIllegal(input, ref pointer, out text))
                    return SegmentCurrentCharType.Illegal;

                // Zjistíme, zda vstupující text je/není povolený z hlediska Permitteds:
                SegmentCurrentCharType permittedCharType;
                if (this.IsPermitted(input, ref pointer, out text, out permittedCharType))
                    return permittedCharType;
            ...
            /// <summary>
            /// Vyhodnocuje pole povolených textů <see cref="PermittedsChar"/> a povolené rodiny znaků <see cref="PermittedFamilies"/>.
            /// Pokud není nic definováno, pak je znak povolený a metoda vrací false a nastavuje out permittedCharType = <see cref="SegmentCurrentCharType.None"/>.
            /// Pokud znak vyhovuje zadání, metoda vrací true a nastavuje out permittedCharType = <see cref="SegmentCurrentCharType.Text"/>..
            /// Pokud znak nevyhovuje, metoda vrací true a nastavuje out permittedCharType = <see cref="SegmentCurrentCharType.EndCurrentSegment"/>.
            /// <para/>
            /// Ověřuje se povolení daného znaku z hlediska <see cref="PermittedFamilies"/> OR <see cref="PermittedsChar"/>.
            /// Pokud je znak povolen, pak se tento znak akceptuje jako vstupující text, vkládá se do výstupu (jako text) a nastavuje se permittedType = Text, vrací se true.
            /// Pokud se znak neakceptuje, pak se text nechá NULL a do permittedType se vloží EndCurrentSegment, vrací se true.
            /// </summary>
            /// <param name="input"></param>
            /// <param name="pointer"></param>
            /// <param name="text"></param>
            /// <param name="permittedCharType"></param>
            /// <returns></returns>
            protected bool IsPermitted(char[] input, ref int pointer, out string text, out SegmentCurrentCharType permittedCharType)
            {
                text = null;
                permittedCharType = SegmentCurrentCharType.None;

                // Pokud tento segment NEobsahuje zadání permitted textů, pak nereagujeme a vracíme false 
                //  = neřešíme Permitted (aktuální znak bude načten jako běžný znak segmentu):
                if (!this.HasPermittedFamilies && !this.HasPermitteds) return false;

                // Řešíme povolené rodiny. Aktuální znak je z povolené rodiny?
                if (this.HasPermittedFamilies && IsMatchingFamily(input, ref pointer, this.PermittedFamilies.Value, out text))
                {   // Povolený znak => jde o součást textu v segmentu:
                    permittedCharType = SegmentCurrentCharType.Text;
                    return true;
                }

                // Řešíme povolené znaky. Aktuální znak je povolený?
                if (this.HasPermitteds && IsMatching(input, ref pointer, this.PermittedsChar, out text))
                {   // Povolený znak (nebo sekvence) => jde o součást textu v aktuálním segmentu:
                    permittedCharType = SegmentCurrentCharType.Text;
                    return true;
                }

                // Nepovolený znak => pointer se neposunul, text obsahuje null: nastavíme typ prvku = Konec segmentu a vrátíme true,
                //  tím bude segment ukončen, a přitom aktuální znak (nepovolený) se zpracuje do dalšího segmentu:
                permittedCharType = SegmentCurrentCharType.EndCurrentSegment;
                return true;
            }
            /// <summary>
            ...
            /// <summary>
            /// Vrátí true, pokud znak na dané pozici vstupu patří do některé ze zadaných families.
            /// Pokud ano, potom znak umístí do out text a navýší pointer.
            /// </summary>
            /// <param name="input"></param>
            /// <param name="pointer"></param>
            ...
            /// <param name="text"></param>
            /// <returns></returns>
            protected static bool IsMatchingFamily(char[] input, ref int pointer, CharacterFamilyType familyType, out string text)
            {
                text = null;
                char c = input[pointer];
                if (!IsMatchingFamily(c, familyType)) return false;
                text = c.ToString();
                pointer++;
                return true;
            }
            /// <summary>
            /// Vrátí true, pokud daný znak patří do některé ze zadaných families
            /// </summary>
            /// <param name="input"></param>
            /// <param name="familyType"></param>
            /// <returns></returns>
            protected static bool IsMatchingFamily(char input, CharacterFamilyType familyType)
            {
                if (familyType.HasFlag(CharacterFamilyType.Control) && Char.IsControl(input)) return true;
                if (familyType.HasFlag(CharacterFamilyType.WhiteSpace) && Char.IsWhiteSpace(input)) return true;
                if (familyType.HasFlag(CharacterFamilyType.Separator) && Char.IsSeparator(input)) return true;
                if (familyType.HasFlag(CharacterFamilyType.Symbol) && Char.IsSymbol(input)) return true;
                if (familyType.HasFlag(CharacterFamilyType.Number) && Char.IsNumber(input)) return true;
                if (familyType.HasFlag(CharacterFamilyType.Digit) && Char.IsDigit(input)) return true;
                if (familyType.HasFlag(CharacterFamilyType.Letter) && Char.IsLetter(input)) return true;
                if (familyType.HasFlag(CharacterFamilyType.LetterOrDigit) && Char.IsLetterOrDigit(input)) return true;
                if (familyType.HasFlag(CharacterFamilyType.Lower) && Char.IsLower(input)) return true;
                if (familyType.HasFlag(CharacterFamilyType.Upper) && Char.IsUpper(input)) return true;
                if (familyType.HasFlag(CharacterFamilyType.Punctuation) && Char.IsPunctuation(input)) return true;
                if (familyType.HasFlag(CharacterFamilyType.Surrogate) && Char.IsSurrogate(input)) return true;
                if (familyType.HasFlag(CharacterFamilyType.HighSurrogate) && Char.IsHighSurrogate(input)) return true;
                if (familyType.HasFlag(CharacterFamilyType.LowSurrogate) && Char.IsLowSurrogate(input)) return true;
                return false;
            }
            /// <summary>
        ...
        /// <param name="setting"></param>
        /// <returns></returns>
        public static ParsedItem ParseText(string code, Setting setting)
        {
        ...
        /// <summary>
        /// Název Settingu, pro který je vytvořen this parser. Vyjadřuje typicky název pravidel, která se používají pro parsování.
        /// Pochází z <see cref="Setting.Name"/> z objektu <see cref="Setting"/>, pro který je vytvořen this parser.
        /// </summary>
        public string SettingName { get { return this._Setting.Name; } }
        /// <summary>
        ...
        protected ParsedItem RunParse(string code)
        {
            ...
            // Instanci Parser je možno používat opakovaně, ale je nezbytné ji před dalším použitím vyčistit:
            this.Clear();
            ...
            while (true)
            {
                ...
                SegmentCurrentCharType change = currentSetting.DetectChange(codeChar, ref currPointer, out currText, out nextSetting);

                // Nějaká forma úplného konce vstupního textu => musíme korektně dokončit všechny rozběhnuté segmenty a skončit:
                if (change == SegmentCurrentCharType.None)
                ...
                switch (change)
                {
                    case SegmentCurrentCharType.BeginNewSegment:
                    ...
                    case SegmentCurrentCharType.Text:
                        ((IParsedItemCreate)currParsedItem).AddText(prevPointer, currText);
                        break;

                    case SegmentCurrentCharType.TextEnd:
                    ...
                    case SegmentCurrentCharType.Blank:
                        ((IParsedItemCreate)currParsedItem).AddBlank(prevPointer, currText);
                        break;

                    case SegmentCurrentCharType.Delimiter:
                        ((IParsedItemCreate)currParsedItem).AddDelimiter(prevPointer, currText);
                        break;

                    case SegmentCurrentCharType.EndCurrentSegment:
                    case SegmentCurrentCharType.StopCurrentSegment:
                        if (change == SegmentCurrentCharType.StopCurrentSegment || currentSetting.EndOutside)
        ...
        /// <summary>
        /// Vynuluje instanční proměnné Parseru, které mohly zůstat naplněny po dokončení předchozího parsování.
        /// Volá se před zahájením parsování.
        /// Dovoluje tak používat jednu instanci parseru opakovaně, bez rizika zanesení chyb vlivem pozůstatků dat po předešlém chodu.
        /// Neprovádí se nulování settingu.
        /// </summary>
        private void Clear()
        {
            if (this._CurrentSettingStack.Count > 0)
        ...
        private void _CheckParseProgress(int lastPointer, int pointer, SegmentCurrentCharType change)
        {
            if (pointer > lastPointer) return;
            if (change == SegmentCurrentCharType.BeginNewSegment || change == SegmentCurrentCharType.EndCurrentSegment) return;
        ...
        /// </summary>
        internal enum SegmentCurrentCharType
        {
            /// <summary>
            /// Není text, konec.
            /// </summary>
        ...
        public static string ScanChangeItems(string expression, Setting setting, Func<ParsedItem, bool> childFilter, Func<ParsedItem, string> textModifier)
        {
            ...
            ParsedItem rootItem = ParseText(expression, setting);  // Parsování vrací VŽDY typ prvku = Array
            rootItem.ScanItems(recurseFilter: childFilter, textModifier: textModifier);
    ...
    internal class ParsedItem : IParsedItemCreate, IParsedItemExtended
    {
        ...
        /// <summary>
        /// Vytvoří a vrátí new instanci <see cref="ParsedItem"/>, typu Array, jehož prvky jsou explicitně dodané.
        /// Pozor, u dodaných prvků se nijak nemění jejich Parent ani Setting, pokud tedy jsou prvky z různých zdrojů, každý si i nadále nese svoje Parenty.
        /// </summary>
        /// <param name="parsedItems"></param>
        /// <returns></returns>
        public static ParsedItem CreateFrom(ParsedItem[] parsedItems)
        {
            if (parsedItems is null || parsedItems.Length == 0 || parsedItems[0] is null) return null;
            ParsedItem first = parsedItems[0];
            SettingSegment setting = first.Root.Setting;
            Parser.SettingSegmentCompiled settingCompiled = first.Root.SettingCompiled;
            ParsedItem rootItem = new ParsedItem(null, setting, settingCompiled, ItemType.Array);
            rootItem._Items.AddRange(parsedItems);
            return rootItem;
        }
        /// <summary>
        ...
        /// </summary>
        protected ItemExpressionState ExpressionState { get; set; }
        /// <summary>
        /// Parent položka, do které this item patří. Root item zde má null.
        /// </summary>
        public ParsedItem Parent { get; private set; }
        /// <summary>
        /// Úroveň prvku: root prvek má <see cref="Level"/> == -1, je to úroveň celého dokumentu (jeho <see cref="Parent"/> je null);
        /// vnořené prvky mají <see cref="Level"/> o 1 vyšší než jejich <see cref="Parent"/>.
        /// Prvky jazyka SQL mají hodnotu <see cref="Level"/> takto:
        /// Vstupní text je členěn na jednotlivé texty oddělené středníkem; celý tento blok má <see cref="Level"/> == 0.
        /// Jednotlivé prvky v této úrovni mají <see cref="Level"/> == 1, např. první keyword "SELECT" ve vstupním parsovaném textu "SELECT * FROM (select * from lcs.tabulka) t" má <see cref="Level"/> == 1,
        /// kdežto keyword "select" použitý v subselectu hned za závorkou má <see cref="Level"/> == 2.
        /// </summary>
        public int Level { get { return (this.Parent != null ? this.Parent.Level + 1 : -1); } }
        /// <summary>
        ...
        /// <summary>
        /// Obsahuje true pokud this segment je komentářem.
        /// Komentáž je z hlediska <see cref="ParsedItem.ItemType"/> = <see cref="NetParser.ItemType.Array"/>, ale z hlediska významu je na úrovni Blank.
        /// </summary>
        public bool IsComment { get { return this.Setting.IsComment; } }
        /// <summary>
        ...
        /// <para/>
        /// Pokud aplikaci nestačí základní property typu <see cref="ParsedItem"/>, nechť si instanci přetypuje na interface 
        /// <see cref="IParsedItemExtended"/>, tam jsou k dispozici další rozšířené property:
        /// <see cref="IParsedItemExtended.TextEdit"/>, <see cref="IParsedItemExtended.BeginText"/>, <see cref="IParsedItemExtended.EndText"/>,
        /// <see cref="IParsedItemExtended.TextModified"/>, <see cref="IParsedItemExtended.SourceText"/> a další...
        /// <para/>
        /// Speciální znaky v této property <see cref="Text"/> jsou nahrazeny jejich významovým obsahem.
        ...
        /// <summary>
        /// Metoda pro všechny dodané prvky <see cref="ParsedItem"/> provede jejich scanování za pomoci dodaných scanovacích metod.
        /// <para/>
        /// V každém dodaném prvku vyhledá všechny prvky typu { Delimiter, Text, Keyword }, a umožní je zpracovat externě dodanou funkcí (parametr textModifier),
        ...
        /// c) neprázdný string = ten bude vložen do textu na dané pozici.
        /// <para/>
        /// Algoritmus metody pro každý jednotlivý prvek:
        /// IF (processFilter) THEN { (action); IF (IsArray) THEN (IF (recurseFilter) THEN (RECURSE INNER ITEMS)) ELSE IF (IsSimpleText) THEN (textModifier) }
        /// </summary>
        /// <param name="parsedItems">Pole prvků</param>
        ...
        /// </param>
        /// <param name="action">Akce, která se provede s daným prvkem. Tato akce nemění obsah prvku, ale dostává jej k dispozici ke svému zpracování.
        /// Na vstupu dostává každý nalezený prvek, pro který bylo vráceno true z metody (parametr) processFilter.
        /// Dostává tedy prvky jak typu Array, tak i <see cref="ParsedItem.IsSimpleText"/> (tedy: Text, Delimiter, Blank).
        /// <para/>
        /// Hodnota "action" může být null.</param>
        ...
        /// Na vstupu dostává nalezený prvek, jehož <see cref="ParsedItem.IsSimpleText"/> == true (tedy: Text, Delimiter, Blank).
        ...
        /// <para/>
        /// Funkce může být null, pak se neprovede.
        /// </param>
        /// <param name="actionArrayEnd">Akce, která se provede při dokončení scanování daného prvku, který je typu Array.
        /// Účelem je dát aplikaci prostor pro zpracování textu na konci prvku.
        /// <para/>
        /// Příklad: mějme prvek typu ZÁVORKA, kde počátek = "(", obsahem prvku je sada textů a oddělovačů (=parametry), a na konci je koncová závorka ")".
        /// Při scanování je vyvolána akce pro prvek typu ZÁVORKA, kde lze řešit <see cref="BeginText"/>, následně jsou scanovány jednotlivé parametry a oddělovače, 
        /// a po scanování posledního vnitřního prvku je volána tato akce (actionArrayEnd), kde aplikace může řešit koncovou závorku dle hodnoty <see cref="EndText"/>.
        /// </param>
        public static void ScanItemsIn(IEnumerable<ParsedItem> parsedItems, Func<ParsedItem, bool> processFilter = null, Action<ParsedItem> action = null, Func<ParsedItem, bool> recurseFilter = null, Func<ParsedItem, string> textModifier = null, Action<ParsedItem> actionArrayEnd = null)
        {
            if (parsedItems == null) return;
            foreach (ParsedItem parsedItem in parsedItems)
                _ScanItems(parsedItem, true, processFilter, action, recurseFilter, textModifier, actionArrayEnd);
        }
        /// <summary>
        ...
        /// IF (processFilter) THEN { (action); IF (IsArray) THEN (IF (recurseFilter) THEN (RECURSE INNER ITEMS)) ELSE IF (IsSimpleText) THEN (textModifier) }
        /// </summary>
        /// <param name="runActionOnItem">Spustit předpesané akce i pro this prvek? true = ano, false = ne, až pro moje ChildItems.</param>
        /// <param name="processFilter">Funkce, která dostává úplně všechny prvky, a rozhoduje se zda provádět jejich další analýzu.
        /// Účelem této funkce je například přeskočit komentáře a blank prvky.
        /// <para/>
        /// Pokud je null, pak se analyzují všechny prvky.
        ...
        /// a po scanování posledního vnitřního prvku je volána tato akce (actionArrayEnd), kde aplikace může řešit koncovou závorku dle hodnoty <see cref="EndText"/>.
        /// </param>
        public void ScanItems(bool runActionOnItem = false, Func<ParsedItem, bool> processFilter = null, Action<ParsedItem> action = null, Func<ParsedItem, bool> recurseFilter = null, Func<ParsedItem, string> textModifier = null, Action<ParsedItem> actionArrayEnd = null)
        {
            // Provedeme _ScanItems pro this prvek, volitelně spustíme akce i pro this prvek (runActionOnItem):
            _ScanItems(this, runActionOnItem, processFilter, action, recurseFilter, textModifier, actionArrayEnd);
        }
        /// <summary>
        /// Provede scanování daného prvku a prvků vnořených, rekurzivně.
        /// </summary>
        /// <param name="parsedItem">Scanovaný prvek</param>
        /// <param name="processFilter">Provést veškeré akce na daném prvku?</param>
        /// <param name="action"></param>
        /// <param name="recurseFilter"></param>
        /// <param name="textModifier"></param>
        /// <param name="actionArrayEnd"></param>
        private static void _ScanItems(ParsedItem parsedItem, bool runActionOnItem, Func<ParsedItem, bool> processFilter = null, Action<ParsedItem> action = null, Func<ParsedItem, bool> recurseFilter = null, Func<ParsedItem, string> textModifier = null, Action<ParsedItem> actionArrayEnd = null)
        {
            ...
            // Pokud this je Simple prvek, pak na něm spustíme akce vždy, protože jde o prostý text / delimiter:
            if (parsedItem.IsSimpleText)
            {
                bool process = (hasProcess ? processFilter(parsedItem) : true);
                if (process)
                {
                    if (hasAction)
                        action(parsedItem);
                    if (hasTextModifier)
                        _ModifyItem(parsedItem, textModifier);
                }
                return;
            }
            if (!parsedItem.IsArray) return;

            // Práce na dodaném prvku typu Array:
            Stack<ScanItemInfo> stack = new Stack<ScanItemInfo>();
            bool hasRecurse = (recurseFilter != null);
            bool hasActionEnd = (actionArrayEnd != null);
            if (runActionOnItem)
            {   // Pokud se mají patřičné akce provést i pro daný prvek:
                bool process = (hasProcess ? processFilter(parsedItem) : true);
                if (process)
                {
                    if (hasAction)
                        action(parsedItem);
                    bool recurse = (hasRecurse ? recurseFilter(parsedItem) : true);
                    if (recurse)
                        stack.Push(new ScanItemInfo(parsedItem));
                }
            }
            else
            {   // Daný prvek sám se nemá řešit, řešit se budou až jeho jednotlivé Items:
                stack.Push(new ScanItemInfo(parsedItem));
            }
            ...
            while (stack.Count > 0)
            {
                ...
                while (true)
                {   // Každý prvek vyjmutý ze zásobníku musím prověřit, i kdyby byl prázdný (tj. neměl svoje Inner prvky):
                    if (scanItem.EndOfChilds)
                    {   // Prvek je na konci Items, anebo ani Items nemá => provedeme akci actionArrayEnd a skončíme smyčku "while (true)":
                        if (hasActionEnd)
                            actionArrayEnd(scanItem.ArrayItem);
                        break;
                    }
                    ...
                    {   // Aktuální Child je Array:
                        ...
                        {   // Pokud budeme zpracovávat toto pole rekurzí, zajistíme to nyní:
                            ...
                            break;                                   // Opustím smyčku scanování while (scanItem..), abych mohl ze zásobníku (stack) vyjmout nový prvek child, v řádku ScanItemInfo scanItem = stack.Pop(); o kousek výše
                        }
                    }
                    else if (childItem.IsSimpleText)
                    {   // Běžný textový prvek:
                        if (hasTextModifier)
                            _ModifyItem(childItem, textModifier);
                    }
                }
            }
        }
        /// <summary>
        /// Zajistí provedení modifikace pro daný prvek
        /// </summary>
        /// <param name="item">Prvek</param>
        /// <param name="textModifier"></param>
        protected static void _ModifyItem(ParsedItem item, Func<ParsedItem, string> textModifier)
        {
            string modifiedText = textModifier(item);
            if (modifiedText != null)
            {   // Pokud výstup není null, pak se bude dělat modifikace:
                IParsedItemCreate iItem = item as IParsedItemCreate;
                if (String.IsNullOrWhiteSpace(modifiedText))
                    // Z textu uděláme Blank:
                    iItem.ChangeTo(ItemType.Blank, modifiedText);
                else
                    // Do prvku typu Text vložíme nový obsah:
                    iItem.ChangeTo(ItemType.Text, modifiedText);
            }
        }
        /// <summary>
        ...
        protected class ScanItemInfo
        {
            /// <summary>
            /// Konstruktor. 
            /// Na vstupu má být výhradně prvek typu <see cref="ItemType.Array"/>.
            /// </summary>
            ...
            public override string ToString()
            {
                return $"LastItem: {(LastItem?.Text ?? "null")}; NextIndex: {NextIndex}; NextItem: {(NextItem?.Text ?? "null")};";
            }
            /// <summary>
            /// Prvek, který byl vrácen při nedávném volání metody <see cref="GetNextChild()"/>.
            /// Obsahuje null před prvním voláním, obsahuje null poté kdy volání <see cref="GetNextChild()"/> vrátilo null.
            /// </summary>
            protected ParsedItem LastItem { get; set; }
            /// <summary>
            /// Prvek, který bude vrácen příště
            /// </summary>
            protected ParsedItem NextItem { get { return (!this.EndOfChilds ? this.ArrayItem._Items[this.NextIndex] : null); } }
            /// <summary>
            ...
            /// Jakmile bude <see cref="NextIndex"/> mimo rozsah počtu prvků pole, bude <see cref="EndOfChilds"/> == true.
            /// </summary>
            ...
            /// </summary>
            public bool EndOfChilds { get { return (this.ArrayItem == null || !this.ArrayItem.IsArray || this.NextIndex < 0 || this.NextIndex >= this.ArrayItem.ItemCount); } }
            /// <summary>
            /// Metoda vrátí prvek <see cref="ArrayItem"/>[<see cref="NextIndex"/>], a poté navýší <see cref="NextIndex"/> o +1.
            ...
            public ParsedItem GetNextChild()
            {
                ...
                ParsedItem childItem = this.ArrayItem._Items[this.NextIndex];
                this.LastItem = childItem;
        ...
        /// <summary>
        /// Pozice znaku ve vstupním textu, který je prvním znakem za tímto segmentem (tedy za posledním znakem textu <see cref="EndText"/>).
        /// Plná délka stringu tohoto segmentu je tedy <see cref="EndPointer"/> - <see cref="BeginPointer"/>.
        /// </summary>
        ...
        ItemExpressionState IParsedItemExtended.ExpressionState { get { return this.ExpressionState; } set { this.ExpressionState = value; } }
    ...
    internal interface IParsedItemExtended
    {
        ...
        /// Výchozí hodnota je None. Konkrétní hodnotu do této property doplňuje zpracovatelský kód.
        /// </summary>
        ItemExpressionState ExpressionState { get; set; }
        /// <summary>
        /// Obsahuje kompletní RTF text tohoto segmentu (včetně hlavičky).
        /// String, který je získán z této property, může být uložen jako soubor.rtf, nebo zobrazen v RtfTextBoxu.
        /// Upozornění: RTF text je určen k zobrazení a k editaci, tedy i k následnému opakovanému parsování.
    ...
    internal enum ItemExpressionState
    {
        ...
        /// Pokud Parser detekuje i Helios Green výrazy a proměnné ( [:Výraz:], :proměnná ), jsou zahrnuty do tohoto typu.
    ...
    internal class ParsedItemBuffer
    {
        ...
        /// <summary>
        /// Aktuální prvek <see cref="CurrentItem"/> typovaný jako <see cref="IParsedItemExtended"/>
        /// </summary>
        public IParsedItemExtended CurrentItemExtended { get { return this.CurrentItem as IParsedItemExtended; } }
        /// <summary>
        ...
        /// <summary>
        /// Příští prvek za prvkem <see cref="CurrentItem"/> typovaný jako <see cref="IParsedItemExtended"/>. Může být null.
        /// </summary>
        public IParsedItemExtended NextItemExtended { get { return (this.HasNextItem ? this.Items[this.Index + 1] as IParsedItemExtended : null); } }
        /// <summary>
        ...
        /// <summary>
        /// Metoda vrátí <see cref="CurrentItem"/> a provede <see cref="Skip()"/>.
        /// Pokud aktuální prvek už není (tj. <see cref="IsEnd"/> == true), vrátí null a Skip neprovádí.
        /// </summary>
        /// <returns></returns>
        public ParsedItem GetCurrentItem()
        {
        ...
        /// Hledá se od slova <see cref="CurrentItem"/> dopředně. 
        /// Pokud se hledané slovo najde, pak se toto nalezené slovo stane aktuálním prvkem <see cref="CurrentItem"/> a vrací se true.
        ...
        /// <returns></returns>
        public ParsedItem[] GetExpression(Dictionary<string, string> stopDict)
        {
            return this.GetExpression(null, stopDict);
        }
        /// <summary>
        ...
        /// <param name="stopDict">Texty, při jejichž detekci se má tvorba výrazu ukončit</param>
        /// <param name="leaveBlank">Ponechat ve vráceném poli i Blank prvky</param>
        /// <returns></returns>
        public ParsedItem[] GetExpression(Dictionary<string, string> joinerDict, Dictionary<string, string> stopDict, bool leaveBlank = false)
        {
            ...
            while (true)
            {
                ...
                this.CurrentItemExtended.ExpressionState = currentState;
                if (result.Count > 0 && leaveBlank && this.IsBlankBeforeCurrent(out ParsedItem blankItem))
        ...
        /// <summary>
        /// Metoda se pokusí najít nejbližší předchozí Blank prvek před prvkem aktuálním.
        /// </summary>
        /// <param name="blankItem"></param>
        /// <returns></returns>
        protected bool IsBlankBeforeCurrent(out ParsedItem blankItem)
        {
            blankItem = null;
            ParsedItem item = this.CurrentItem;
            ParsedItem parent = item.Parent;

            IParsedItemExtended iItem = item as IParsedItemExtended;
            int itemIndex = iItem.ItemIndex;          // Toto je index aktuálního prvku v nativním poli prvků jeho Parenta, na rozdíl od zdejšího indexu (ten je pouze pro RelevantItems)
            while (itemIndex > 0)
            {
                itemIndex--;
                ParsedItem testItem = parent.Items[itemIndex];
                if (testItem.ItemType == ItemType.Blank)
                {   // Našli jsme Blank prvek => ten beru:
                    blankItem = testItem;
                    return true;
                }
                if (((IParsedItemExtended)testItem).IsRelevant)
                    // Našli jsme Relevantní prvek => to znamená, že před naším prvkem žádný Blank prvek není:
                    break;
            }
            // Když jsme nenašli Blank, anebo jsme dojeli na začátek pole:
            return false;
        }
        /// <summary>
        ...
        private static ItemExpressionState GetItemState(ParsedItem currItem, ParsedItem nextItem, Dictionary<string, string> joinerDict, Dictionary<string, string> stopDict)
        {
            ...
            switch (currItem.ItemType)
            {
                ...
                case ItemType.Array:
                    switch (currItem.SegmentName)
                    {
                        case DefaultSettings.SQL_PARENTHESIS: return ItemExpressionState.Parentheses;
                        case DefaultSettings.SQL_STRING: return ItemExpressionState.Value;
                        case DefaultSettings.SQL_STRING_UNICODE: return ItemExpressionState.Value;
                        case DefaultSettings.SQL_VARIABLE: return ItemExpressionState.Value;
                        case DefaultSettings.SQL_GREEN_EXPRESSION: return ItemExpressionState.Value;
                        case DefaultSettings.SQL_GREEN_VARIABLE: return ItemExpressionState.Value;
                    }
    ...
    internal class SettingSegment : ISettingSegmentInternal
    {
        ...
        /// Znak se vyhodnocuje typicky pomocí metod typu: <see cref="Char.IsDigit(char)"/> pro <see cref="CharacterFamilyType.Digit"/>.
        /// Pokud je null, nevyhodnocuje se.
        /// Souběh zadání <see cref="PermittedFamilies"/> a <see cref="Permitteds"/> je operátorem OR, 
        /// tzn. pokud znak vyhovuje alespoň z hlediska Family, anebo je uveden v povolených znacích, pak je akceptován jako povolený znak segmentu.
        /// Lze tak povolit všechny číslice a písmena + přidat znak "_".
        /// </summary>
        public CharacterFamilyType? PermittedFamilies { get; set; }
        /// <summary>
    ...
    {
        #region Generátor settings pro MsSql a MsSqlGreen
        /// <summary>
        /// Setting pro Microsoft T-SQL.
        /// Tento setting je vhodný pro analýzu kódu, ale NEOBSAHUJE formátovací předpisy = není určen pro SyntaxColoring.
        /// Důvodem je rychlost tvorby: Setting bez formátů je připraven za 4 milisec, setting včetně formátů trvá cca 6 milisekund.
        /// </summary>
        public static Setting MsSql { get { return _GetSettingMsSqlStore(SQL_NAME, false, true, false, ref _MsSql); } }
        private static Setting _MsSql;
        /// <summary>
        /// Setting pro Microsoft T-SQL včetně podkladů pro obarvení kódu při editaci.
        /// Důvodem je rychlost tvorby: Setting bez formátů je připraven za 4 milisec, setting včetně formátů trvá cca 6 milisekund.
        /// </summary>
        public static Setting MsSqlColor { get { return _GetSettingMsSqlStore(SQL_NAME, true, true, false, ref _MsSqlColor); } }
        private static Setting _MsSqlColor;
        /// <summary>
        /// Setting pro Microsoft T-SQL plus podpora výrazů Helios Green, ale NEOBSAHUJE formátovací předpisy = není určen pro SyntaxColoring.
        /// Důvodem je rychlost tvorby: Setting bez formátů je připraven za 4 milisec, setting včetně formátů trvá cca 6 milisekund.
        /// </summary>
        public static Setting MsSqlGreen { get { return _GetSettingMsSqlStore(SQL_GREEN_NAME, false, true, true, ref _MsSqlGreen); } }
        private static Setting _MsSqlGreen;
        /// <summary>
        /// Setting pro Microsoft T-SQL plus podpora výrazů Helios Green, včetně podkladů pro obarvení kódu při editaci.
        /// Důvodem je rychlost tvorby: Setting bez formátů je připraven za 4 milisec, setting včetně formátů trvá cca 6 milisekund.
        /// </summary>
        public static Setting MsSqlGreenColor { get { return _GetSettingMsSqlStore(SQL_GREEN_NAME, false, true, true, ref _MsSqlGreenColor); } }
        private static Setting _MsSqlGreenColor;
        /// <summary>
        /// Vrací setting ze store
        /// </summary>
        /// <param name="name"></param>
        /// <param name="addSyntaxFormat"></param>
        /// <param name="enableMergeDotSpaceText"></param>
        /// <param name="enableGreenSegments"></param>
        /// <param name="store"></param>
        /// <returns></returns>
        private static Setting _GetSettingMsSqlStore(string name, bool addSyntaxFormat, bool enableMergeDotSpaceText, bool enableGreenSegments, ref Setting store)
        {
            if (store == null)
                store = _GetSettingMsSql(name, addSyntaxFormat, enableMergeDotSpaceText, enableGreenSegments);
            return store;
        }
        /// <summary>
        /// Vytvoří setting podle požadavků.
        /// </summary>
        /// <param name="name"></param>
        /// <param name="addSyntaxFormat"></param>
        /// <param name="enableMergeDotSpaceText"></param>
        /// <param name="enableGreenSegments"></param>
        /// <returns></returns>
        private static Setting _GetSettingMsSql(string name, bool addSyntaxFormat, bool enableMergeDotSpaceText, bool enableGreenSegments)
        {
            Setting setting = new Setting(name, SQL_CODE);
            _CreateSettingMsSql(setting, addSyntaxFormat, enableMergeDotSpaceText, enableGreenSegments);
            return setting;
        }
        /// <summary>
        ...
        /// </summary>
        /// <param name="setting"></param>
        /// <param name="enableMergeDotSpaceText">Zajistit povolení EnableMergeTextBlankDotText a EnableMergeTextSpaceDotAsterix</param>
        /// <param name="enableGreenSegments">Vložit segmenty, charakteristické pro výrazy Helios Green</param>
        private static SettingSegment _CreateSettingMsSql(Setting setting, bool addSyntaxFormat, bool enableMergeDotSpaceText, bool enableGreenSegments)
        {
            SettingSegment segment;
            SettingSegment result;

            string greenBlankQuotation = (enableGreenSegments ? "\"" : null);     // Výraz Greenu smí obsahovat uvozovku, která se chápe jako Blank
            string greenSegmentExpression = (enableGreenSegments ? SQL_GREEN_EXPRESSION : null);
            string greenSegmentVariable = (enableGreenSegments ? SQL_GREEN_VARIABLE : null);

            // Společná data pro SQL_CODE a SQL_PARENTHESIS, které obě mohou obsahovat SQL kód:
            string[] blanks = CreateValidArray(" ", "\t", "\r", "\n", greenBlankQuotation);
            string[] delimiters = CreateValidArray(",", "+", "-", "*", "/", "<", "=", ">", "<=", ">=", "<>", "%");
            ...
            segment.InnerSegmentsNames = CreateValidArray(SQL_STRING, SQL_STRING_UNICODE, SQL_SYSNAME, SQL_VARIABLE, greenSegmentExpression, greenSegmentVariable, SQL_PARENTHESIS, SQL_COMMENTLINE, SQL_COMMENTBLOCK);
            ...
            if (enableGreenSegments)       // Syntaxe Helios Green povoluje výskyt EXPRESSION i uvnitř stringů:
                segment.InnerSegmentsNames = CreateValidArray(greenSegmentExpression);
            if (addSyntaxFormat)
            ...
            // SQL proměnná @variable:
            segment = new SettingSegment(SQL_VARIABLE);
            segment.BeginWith = "@";
            segment.PermittedFamilies = CharacterFamilyType.Letter | CharacterFamilyType.Number;
            segment.Permitteds = SplitTextToOneCharStringArray("_");
            if (addSyntaxFormat)
                ApplySchemeSysObject(segment);
            setting.SegmentSettingAdd(segment);

            // Helios Green:
            if (enableGreenSegments)
            {
                // Výraz v Greenu = [:text(parametr)|atribut=hodnota:]
                segment = new SettingSegment(SQL_GREEN_EXPRESSION);
                segment.BeginWith = "[:";
                segment.Delimiters = CreateValidArray(",");
                segment.EndWith = CreateValidArray(":]", ":)");
                segment.InnerSegmentsNames = CreateValidArray(SQL_GREEN_PARAMETER);
                if (addSyntaxFormat)
                    ApplySchemeCode(segment);
                setting.SegmentSettingAdd(segment);

                // Proměnná Helios Green = začíná dvojtečkou, obsahuje text a číslice a podtržítko a tečku, vnořené parametry, ale ne mezeru a nic dalšího:
                segment = new SettingSegment(SQL_GREEN_VARIABLE);
                segment.BeginWith = ":";
                segment.PermittedFamilies = CharacterFamilyType.LetterOrDigit;
                segment.Permitteds = CreateValidArray("_", ".", "~");
                segment.InnerSegmentsNames = CreateValidArray(SQL_GREEN_PARAMETER);
                if (addSyntaxFormat)
                    ApplySchemeCode(segment);
                setting.SegmentSettingAdd(segment);

                // Parametry v závorce Green = mohou se nacházet ve Výrazu SQL_GREEN_EXPRESSION i v Proměnné SQL_GREEN_VARIABLE,
                //  mohou obsahovat texty oddělené čárkou, mohou obsahovat stringy SQL_GREEN_STRING:
                //  uvozené apostrofem (v nich pak nedetekujeme ani čárku, ani koncovou závorku):
                segment = new SettingSegment(SQL_GREEN_PARAMETER);
                segment.BeginWith = "(";
                segment.Blanks = blanks.ToArray();
                segment.Delimiters = CreateValidArray(",");
                segment.EndWith = CreateValidArray(")");
                segment.InnerSegmentsNames = CreateValidArray(SQL_GREEN_STRING);
                if (addSyntaxFormat)
                    ApplySchemeCode(segment);
                setting.SegmentSettingAdd(segment);

                // Stringy, uvedené jako parametr v závorce GREEN výrazu nebo promněnné:
                //  Začínají a končí apostrofem, uvnitř neobsahujíc nic jiného, mohou obsahovat speciální znak = double-apostrof:
                segment = new SettingSegment(SQL_GREEN_STRING);
                segment.BeginWith = "'";
                segment.EndWith = new string[] { "'" };
                segment.SpecialTexts = new SpecialTexts[] { new SpecialTexts("''", "'") };
                if (addSyntaxFormat)
        ...
        /// </summary>
        /// <returns></returns>
        private static List<string> GetKeywordsSqlLanguage()
        {
...
";
        ...
        /// <summary>
        /// Název pro pravidla parsování jazyka SQL
        /// </summary>
        public const string SQL_NAME = "MsSql";
        /// <summary>
        /// Název pro pravidla parsování jazyka SQL s rozšířením o prvky Helios Green
        /// </summary>
        public const string SQL_GREEN_NAME = "MsSqlGreen";
        /// <summary>
        /// Název parsovaného segmentu, který představuje prostý SQL příkaz v základní úrovni textu, ne v závorce.
        /// </summary>
        public const string SQL_CODE = "SqlCode";
        /// <summary>
        /// Název parsovaného segmentu, který představuje string:
        /// začíná ', končí ' a uvnitř může obsahovat '' které reprezentují jeden '
        /// </summary>
        public const string SQL_STRING = "SqlString";
        /// <summary>
        /// Název parsovaného segmentu, který představuje UNICODE string:
        /// začíná N', končí ' a uvnitř může obsahovat '' které reprezentují jeden '
        /// </summary>
        public const string SQL_STRING_UNICODE = "SqlStringUnicode";
        /// <summary>
        /// Název parsovaného segmentu, který představuje systémové jméno:
        /// začíná [ a končí ]
        /// </summary>
        public const string SQL_SYSNAME = "SqlSysName";
        /// <summary>
        /// Název parsovaného segmentu, který představuje SQL proměnnou:
        /// začíná @ a obsahuje jen čísla, písmena a podtržítka.
        /// </summary>
        public const string SQL_VARIABLE = "SqlVariable";
        /// <summary>
        /// Název parsovaného segmentu, který představuje text v závorce.
        /// Na jeho začátku je levá kulatá závorka "(", uvnitř je cokoliv, včetně i dalších závorek, na konci je pravá kulatá závorka ")".
        /// </summary>
        public const string SQL_PARENTHESIS = "SqlParenthesis";
        /// <summary>
        /// Název parsovaného segmentu, který představuje řádkový komentář: 
        /// začíná dvěma pomlčkami a končí koncem řádku.
        /// </summary>
        public const string SQL_COMMENTLINE = "SqlCommentLine";
        /// <summary>
        /// Název parsovaného segmentu, který představuje blokový komentář:
        /// začíná /*, obsahuje cokoliv, končí */
        /// </summary>
        public const string SQL_COMMENTBLOCK = "SqlCommentBlock";
        /// <summary>
        /// Název parsovaného segmentu, který představuje GREEN výraz:
        /// začíná [: a končí :], může obsahovat text v závorce, segment jména <see cref="SQL_GREEN_PARAMETER"/>.
        /// </summary>
        public const string SQL_GREEN_EXPRESSION = "SqlGreenExpression";
        /// <summary>
        /// Název parsovaného segmentu, který představuje GREEN proměnnou:
        /// začíná : a obsahuje písmena a číslice, může obsahovat text v závorce, segment jména <see cref="SQL_GREEN_PARAMETER"/>.
        /// </summary>
        public const string SQL_GREEN_VARIABLE = "SqlGreenVariable";
        /// <summary>
        /// Název parsovaného segmentu, který představuje GREEN parametry v závorce (tj. závorka uvnitř výrazu nebo proměnné GREEN).
        /// </summary>
        public const string SQL_GREEN_PARAMETER = "SqlGreenParameter";
        /// <summary>
        /// Název parsovaného segmentu, který představuje GREEN string = string v závorce v GREEN výrazu nebo proměnné.
        /// Dokážeme tak parsovat i tento text: "..... [:Vyhledej(150, 'Funkce hlavičky: :]'):] ...",
        /// a zajistit tak nalezení názvu funkce "Vyhledej" a dvou parametrů: 150 a "Funkce hlavičky: :]". 
        /// Pak končí parametry ")" a pak končí výraz ":]".
        /// </summary>
        public const string SQL_GREEN_STRING = "SqlGreenString";
        #endregion
        #endregion
        ...
        private static Setting _GetSettingRtf()
        {
            ...
            segment.Permitteds = SplitTextToOneCharStringArray(NUMERICS + ASCIICHARS + "*");      // Entita smí obsahovat jen číslice a malá a velká základní písmena a *
            ...
            segment.BeginWith = @"\'";
            segment.Permitteds = SplitTextToOneCharStringArray(HEXADECIMALS);
        ...
        private static Setting _GetSettingCSharp()
        {
            ...
            segment.Blanks = blanks.ToArray();
            segment.Delimiters = delimiters.ToArray();
            segment.InnerSegmentsNames = new string[] { CS_BLOCK, CS_STRING, CS_STRING_RAW, CS_STRING_INSET, CS_STRING_RAWINSET, CS_CHAR, CS_PARENTHESIS, CS_INDEX, CS_COMMENT_BLOCK, CS_COMMENT_ROW, CS_XML_COMMENT, CS_REGION };
            segment.Keywords = keywordList.ToArray();
            segment.KeywordsCaseSensitive = true;
            segment.EnableMergeTextBlankDotText = true;
            ApplySchemeCode(segment);
            ...
            // INSET string začíná $" a končí ", obsahuje speciální znaky, může obsahovat segment CS_BLOCK:
            segment = new SettingSegment(CS_STRING_INSET);
            segment.BeginWith = "$\"";
            segment.EndWith = new string[] { "\"", "\r\n", "\r", "\n" };
            segment.SpecialTexts = specs.ToArray();
            segment.InnerSegmentsNames = new string[] { CS_BLOCK };
            ApplySchemeString(segment);
            setting.SegmentSettingAdd(segment);

            // RAW + INSET string začíná @$" a končí ", neobsahuje žádné speciální znaky, může obsahovat segment CS_BLOCK:
            segment = new SettingSegment(CS_STRING_RAWINSET);
            segment.BeginWith = "@$\"";
            segment.EndWith = new string[] { "\"" };
            segment.InnerSegmentsNames = new string[] { CS_BLOCK };
            ApplySchemeString(segment);
        ...
        /// </summary>
        /// <returns></returns>
        private static List<string> GetKeywordsCSharpLanguage()
        {
            #region Text of keywords : seznam klíčových slov pochází z článku SQL Helpu, a lze jej odtamtud snadno aktualizovat
            string keywords = @"
...
";
        ...
        /// <summary>
        /// Název segmentu v parseru C#: obsah dokumentu na úrovni souboru (začíná např. using)
        /// </summary>
        public const string CS_DOCUMENT = "CsDocument";
        /// <summary>
        /// Název segmentu v parseru C#: obsah bloku { ... }
        /// </summary>
        public const string CS_BLOCK = "CsBlock";
        /// <summary>
        /// Název segmentu v parseru C#: obsah v kulatých závorkách ( ... )
        /// </summary>
        public const string CS_PARENTHESIS = "CsParenthesis";
        /// <summary>
        /// Název segmentu v parseru C#: obsah v hranatých závorkách [ ... ] = indexer
        /// </summary>
        public const string CS_INDEX = "CsIndex";
        /// <summary>
        /// Název segmentu v parseru C#: komentář blokový /* ... */
        /// </summary>
        public const string CS_COMMENT_BLOCK = "CsCommentBlock";
        /// <summary>
        /// Název segmentu v parseru C#: komentář řádkový // ... EOL
        /// </summary>
        public const string CS_COMMENT_ROW = "CsCommentRow";
        /// <summary>
        /// Název segmentu v parseru C#: #region
        /// </summary>
        public const string CS_REGION = "CsRegion";
        /// <summary>
        /// Název segmentu v parseru C#: XML komentář /// ... EOL
        /// </summary>
        public const string CS_XML_COMMENT = "CsXmlComment";
        /// <summary>
        /// Název segmentu v parseru C#: character 'X'
        /// </summary>
        public const string CS_CHAR = "CsChar";
        /// <summary>
        /// Název segmentu v parseru C#: běžný string " ... "
        /// </summary>
        public const string CS_STRING = "CsString";
        /// <summary>
        /// Název segmentu v parseru C#: string přes více řádků @" ... EOL ..."
        /// </summary>
        public const string CS_STRING_RAW = "CsStringRaw";
        /// <summary>
        /// Název segmentu v parseru C#: string obsahující vkládané proměnné $"Text {variable}", 
        /// kde variable je standardní kód <see cref="CS_BLOCK"/>
        /// </summary>
        public const string CS_STRING_INSET = "CsStringInset";
        /// <summary>
        /// Název segmentu v parseru C#: string obsahující vkládané proměnnépřes více řádků @$" Text {proměnná} ... EOL další řádek {proměnná} ...", 
        /// kde variable je standardní kód <see cref="CS_BLOCK"/>
        /// </summary>
        public const string CS_STRING_RAWINSET = "CsStringRawInset";
        #endregion
        #endregion
        ...
        private static Color SchemePlainColorText { get { return Color.Black; } }
        /// <summary>Barva pro text: Text kódu</summary>
        private static Color SchemeCodeColorText { get { return Color.DarkSlateGray; } }
        /// <summary>Barva pro text: Klíčové slovo</summary>
        private static Color SchemeKeywordColorText { get { return Color.MediumBlue; } }
        /// <summary>Barva pro text: Název systémového objektu</summary>
        private static Color SchemeSysObjectColorText { get { return Color.DarkSlateGray; } }

        /// <summary>Barva pro text: Oddělovače</summary>
        private static Color SchemeDelimiterColorText { get { return Color.Magenta; } }

        /// <summary>Barva pro text: Proměnná</summary>
        private static Color SchemeVariableColorText { get { return Color.DarkCyan; } }
        /// <summary>Styl písma pro : Proměnná, počátek</summary>
        private static FormatFontStyle SchemeVariableStyleBegin { get { return FormatFontStyle.None; } }
        /// <summary>Styl písma pro : Proměnná, konec</summary>
        private static FormatFontStyle SchemeVariableStyleEnd { get { return FormatFontStyle.None; } }

        /// <summary>Barva pro text: String</summary>
        private static Color SchemeStringColorText { get { return Color.DarkRed; } }
        /// <summary>Styl písma pro : String, počátek</summary>
        private static FormatFontStyle SchemeStringStyleBegin { get { return FormatFontStyle.Bold; } }
        /// <summary>Styl písma pro : String, konec</summary>
        private static FormatFontStyle SchemeStringStyleEnd { get { return FormatFontStyle.BoldEnd; } }

        /// <summary>Barva pro text: Char</summary>
        private static Color SchemeCharColorText { get { return Color.DarkViolet; } }
        /// <summary>Styl písma pro : Char, počátek</summary>
        private static FormatFontStyle SchemeCharStyleBegin { get { return FormatFontStyle.Bold; } }
        /// <summary>Styl písma pro : Char, konec</summary>
        private static FormatFontStyle SchemeCharStyleEnd { get { return FormatFontStyle.BoldEnd; } }

        /// <summary>Barva pro text: Entity (speciální znaky)</summary>
        private static Color SchemeEntityColorText { get { return Color.DarkBlue; } }
        /// <summary>Barva pro pozadí: Entity (speciální znaky)</summary>
        private static Color SchemeEntityColorBack { get { return Color.AliceBlue; } }
        /// <summary>Styl písma pro : Entity (speciální znaky), počátek</summary>
        private static FormatFontStyle SchemeEntityStyleBegin { get { return FormatFontStyle.None; } }
        /// <summary>Styl písma pro : Entity (speciální znaky), konec</summary>
        private static FormatFontStyle SchemeEntityStyleEnd { get { return FormatFontStyle.None; } }

        /// <summary>Barva pro text: Komentář (barva písma)</summary>
        private static Color SchemeCommentColorText { get { return Color.DarkSlateGray; } }
        /// <summary>Barva pro text: Komentář obecně (barva pozadí)</summary>
        private static Color SchemeCommentColorBack { get { return Color.FromArgb(226, 226, 234); } }
        /// <summary>Barva pro text: Komentář blokový (barva pozadí)</summary>
        private static Color SchemeCommentBlockColorBack { get { return SchemeCommentColorBack; } }
        /// <summary>Barva pro text: Komentář řádkový (barva pozadí)</summary>
        private static Color SchemeCommentRowColorBack { get { return SchemeCommentColorBack; } }
        /// <summary>Barva pro text: Komentář XML (barva pozadí)</summary>
        private static Color SchemeCommentXmlColorBack { get { return Color.AliceBlue; } }
        /// <summary>Styl písma pro : Komentář, počátek</summary>
        private static FormatFontStyle SchemeCommentStyleBegin { get { return FormatFontStyle.None; } }
        /// <summary>Styl písma pro : Komentář, konec</summary>
        private static FormatFontStyle SchemeCommentStyleEnd { get { return FormatFontStyle.None; } }
        ...
        #region Služby
        /// <summary>
        /// Vrátí pole vytvořené z dodaných prvků, které nejsou Empty
        /// </summary>
        /// <param name="input"></param>
        /// <returns></returns>
        private static string[] CreateValidArray(params string[] input)
        {
        ...
        /// <returns></returns>
        private static List<string> SplitTextToRows(string keywords, params char[] separators)
        {
        ...
        /// <param name="text"></param>
        /// <returns></returns>
        private static string[] SplitTextToOneCharStringArray(string text)
        {
        ...
        /// <summary>
        /// Hexadecimální znaky 0-9  a-f  A-F
        /// </summary>
        private const string HEXADECIMALS = "0123456789abcdefABCDEF";
        /// <summary>
        /// Malá písmena a-z
        /// </summary>
        private const string ASCIICHARS_LOWER = "abcdefghijklmnopqrstuvwxyz";
        /// <summary>
        /// Velká písmena A-Z
        /// </summary>
        private const string ASCIICHARS_UPPER = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        /// <summary>
        /// Všechna písmena A-Z  a-z
        /// </summary>
        private const string ASCIICHARS = ASCIICHARS_UPPER + ASCIICHARS_LOWER;
        #endregion
    }
    #region Enums
    /// <summary>
    /// Podmnožina znaků v rozsahu Char
    /// </summary>
    [Flags]
    internal enum CharacterFamilyType
    {
        /// <summary>
        /// None
        /// </summary>
        ...
        Punctuation = 0x0400,
        /// <summary>
        /// character is either a high surrogate or a low surrogate
        /// </summary>
        Surrogate = 0x1000,
        /// <summary>
        /// numeric value of the specified character ranges from U+D800 through U+DBFF
        /// </summary>
        HighSurrogate = 0x2000,
        /// <summary>
        /// numeric value of the specified character ranges from U+DC00 through U+DFFF
        /// </summary>
        LowSurrogate = 0x4000
    }
    #endregion
