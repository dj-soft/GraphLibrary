...
namespace Noris.Clients.Win.Components.AsolDX.DataForm
{
    ...
    internal static class DfTemplateLayout
    {
        ...
        private static void _CreateLayout(DfTemplateLayoutArgs args)
        {
            ...
            {
                DfTemplateLayoutStyle styleForm = new DfTemplateLayoutStyle(dfForm);
                foreach (var dfPage in dfForm.Pages)
                {
                    if (dfPage != null && dfPage.Panels != null)
                    {
                        DfTemplateLayoutStyle stylePage = new DfTemplateLayoutStyle(dfPage, styleForm);
                        ...
                        {   // Pro panel budu počítat rozmístění jeho vnitřních prvků a následně i vnější rozměry panelu.
                            ...
                            DfTemplateLayoutStyle stylePanel = new DfTemplateLayoutStyle(dfPanel, stylePage);
                            using (var panelItem = DfTemplateLayoutItem.CreateRoot(dfPanel, args, stylePanel))
                            {
        ...
        private static void _CreateLayoutPanel(DfTemplateLayoutArgs args, DfPanel dfPanel, DfTemplateLayoutStyle styleContainer)
        {
        ...
        /// <returns></returns>
        private static ContainerSize GetEmptyDesignSize(DfBaseContainer dfContainer, DfTemplateLayoutStyle containerStyle)
        {
        ...
        /// <returns></returns>
        private static ContainerSize GetDesignSize(DfBaseContainer dfContainer, DfTemplateLayoutStyle containerStyle, int maxX, int maxY)
        {
            return new ContainerSize(0, 0, 0, 0);
        }
        #endregion
        ...
        private static void _ValidateControlSize(IDataFormItem dataFormItem)
        {
            var controlInfo = dataFormItem.ControlInfo;
            bool hasWidth = controlInfo.DesignWidthPixel.HasValue || controlInfo.DesignWidthPercent.HasValue || controlInfo.ImplicitMinimalWidth.HasValue || controlInfo.ImplicitOptimalWidth.HasValue;
            bool hasHeight = controlInfo.DesignHeightPixel.HasValue || controlInfo.DesignHeightPercent.HasValue || controlInfo.ImplicitMinimalHeight.HasValue || controlInfo.ImplicitOptimalHeight.HasValue;
            ...
            switch (controlInfo.ControlType)
            {
                ...
                case DataControlType.Label: setControlTextSize(12, 1, true); break;
                case DataControlType.Button: setControlTextSize(18, 6, false); break;
                case DataControlType.BarCode: setControlSize(32, 32, false); break;
                case DataControlType.Image: setControlSize(32, 32, false); break;
                case DataControlType.EditBox: setControlSize(250, 40, false); break;
                case DataControlType.TextBox:
                    string name = GetItemKey(dataFormItem.ColumnName, "");
            ...
            {
                string text = controlInfo.Text;
                int width = TextDimension.GetTextWidth(text, controlInfo.Font) + addWidth;
                int height = TextDimension.GetFontHeight(controlInfo.Font) + addHeight;
            ...
            void setControlSize(int? width, int? height, bool asOptimalDimension)
            {
                if (!hasWidth)
                {
                    if (asOptimalDimension)
                        controlInfo.ImplicitOptimalWidth = width;
                    else
                        controlInfo.ImplicitMinimalWidth = width;
                }
                if (!hasHeight)
                {
                    if (asOptimalDimension)
                        controlInfo.ImplicitOptimalHeight = height;
                    else
                        controlInfo.ImplicitMinimalHeight = height;
                }
            }
        }
        /// <summary>
        ...
        /// </summary>
        /// <param name="dataFormItem"></param>
        private static void _ValidateLabelSize(IDataFormItem dataFormItem)
        {
            var mainLabelInfo = dataFormItem.MainLabelInfo;
            if (dataFormItem.LabelPosition != LabelPositionType.None && mainLabelInfo != null)
                checkTextSize(mainLabelInfo);

            var suffixLabelInfo = dataFormItem.SuffixLabelInfo;
            if (suffixLabelInfo != null)
                checkTextSize(suffixLabelInfo);

            void checkTextSize(LabelInfo labelInfo)
            {
                if (labelInfo is null || String.IsNullOrEmpty(labelInfo.Text)) return;

                bool hasWidth = labelInfo.MeasuredWidth.HasValue;
                bool hasHeight = labelInfo.MeasuredHeight.HasValue;
                if (hasWidth && hasHeight) return;

                // Šířka:
                if (!hasWidth) labelInfo.MeasuredWidth = TextDimension.GetTextWidth(labelInfo.Text, labelInfo.Font);

                // Výška:
                if (!hasHeight) labelInfo.MeasuredHeight = TextDimension.GetFontHeight(labelInfo.Font);
            }
        }
        /// <summary>
    ...
    #region class DfTemplateLayoutStyle : Styl vzhledu pro jeden container, podporuje dědičnost
    /// <summary>
    /// Styl vzhledu pro jeden container.
    /// Je uložen v Containeru a určuje parametry pro layout jeho Child prvků (sloupce, mezery, okraje, AreaBegin, LabelOffsety, implicitní pozici labelů pro Child prvky).
    /// Upozornění: implicitní pozici labelu pro samotný this container nelze brát z <see cref="ChildsAutoLabelPosition"/> ze stylu tohoto containeru, protože ta určuje pozici labelu pro jeho Child prvky!
    /// Ta se má brát ze stylu Parent containeru!
    /// Podporuje dědičnost
    /// </summary>
    internal class DfTemplateLayoutStyle
    {
        public DfTemplateLayoutStyle()
        {
            this.CurrentAreaType = ContainerType.None;
            this.ColumnsCount = 0;
            this.ColumnWidths = null;
            this.FlowAreaBegin = null;
            this.CurrentAutoLabelPosition = LabelPositionType.None;
            this.ChildsAutoLabelPosition = LabelPositionType.None;
            this.Margins = null;
        ...
        public DfTemplateLayoutStyle(ContainerType areaType, int? columnsCount, string columnWidths, Location flowAreaBegin, Margins controlMargins, LabelPositionType currentAutoLabelPosition, LabelPositionType childsAutoLabelPosition, Margins margins, int columnsDistance, int rowsDistance, int topLabelOffsetX, int bottomLabelOffsetX, bool labelsRelativeToControl)
        {
            this.CurrentAreaType = areaType;
            this.ColumnsCount = columnsCount;
            this.ColumnWidths = columnWidths;
            this.FlowAreaBegin = flowAreaBegin;
            this.CurrentAutoLabelPosition = currentAutoLabelPosition;
            this.ChildsAutoLabelPosition = childsAutoLabelPosition;
        ...
        public DfTemplateLayoutStyle(DfBaseArea dfArea)
        {
            ...
            this.CurrentAutoLabelPosition = LabelPositionType.None;
            this.CurrentAutoLabelPosition = LabelPositionType.None;
            this.ChildsAutoLabelPosition = dfArea.AutoLabelPosition ?? LabelPositionType.None;
        ...
        public DfTemplateLayoutStyle(DfBaseArea dfArea, DfTemplateLayoutStyle styleParent)
        {
            this.CurrentAreaType = dfArea.AreaType;
            ...
            // Některé hodnoty se výhradně dědí, z Parent.Child do Current:
            this.CurrentAutoLabelPosition = styleParent.ChildsAutoLabelPosition;

            // Ostatní hodnoty jsou "jednomístné" a lze je dědit jednoduše:
            this.ChildsAutoLabelPosition = dfArea.AutoLabelPosition.HasValue ? dfArea.AutoLabelPosition.Value : styleParent.ChildsAutoLabelPosition;
        ...
        /// <summary>
        /// Automaticky generovat labely atributů a vztahů, jejich umístění, pro <u><b>aktuální container a jeho label</b></u> (využije se jen pro grupu).
        /// Defaultní = <c>NULL</c>
        /// </summary>
        public LabelPositionType CurrentAutoLabelPosition { get; private set; }
        /// <summary>
        /// Automaticky generovat labely atributů a vztahů, jejich umístění, pro <u><b>Child prvky</b></u> v tomto layoutu.
        /// Defaultní = <c>NULL</c>
        /// </summary>
        public LabelPositionType ChildsAutoLabelPosition { get; private set; }
        /// <summary>
    ...
    #endregion
    #region class DfTemplateLayoutItem : Dočasná pracovní a výkonná schránka na jednotlivý prvek layoutu, existuje jen po dobu výpočtu layoutu. Má tři tváře: Prvek/Container; Spolupráce s aplikací na doplnění hodnot; Spolupráce s FlowLayout na umístění do mřížky
    /// <summary>
    ...
    /// </summary>
    internal class DfTemplateLayoutItem : IDataFormItem, IFlowLayoutItem, IDisposable
    {
        ...
        /// <param name="style">Styl pro Child prvky daného panelu.</param>
        /// <returns></returns>
        internal static DfTemplateLayoutItem CreateRoot(DfBase dfItem, DfTemplateLayoutArgs dfArgs, DfTemplateLayoutStyle style)
        {
            var rootItem = new DfTemplateLayoutItem(dfItem, null, dfArgs, style);
        ...
        private void _CreateChilds()
        {
            ...
            {
                __Childs = new List<DfTemplateLayoutItem>();
                ...
                foreach (var dfChild in dfChilds)
                {
                    if (dfChild != null)
                    {
                        ...
                        {
                            DfTemplateLayoutStyle childStyle = new DfTemplateLayoutStyle(dfChildContainer, this.__Style);
                            DfTemplateLayoutItem childContainer = new DfTemplateLayoutItem(dfChild, this, null, childStyle);
                        ...
                        {
                            DfTemplateLayoutItem childControl = new DfTemplateLayoutItem(dfChild, this, null, null);
        ...
        private DfTemplateLayoutItem(DfBase dfItem, DfTemplateLayoutItem parent, DfTemplateLayoutArgs dfArgs, DfTemplateLayoutStyle style)
        {
            __DfItem = dfItem;
            __Parent = parent;
            __DfArgs = dfArgs;
        ...
        /// </summary>
        private DfTemplateLayoutItem __Parent;
        /// <summary>
        ...
        /// <summary>
        /// Styl pro tento kontejner, deklaruje prostor určený pro Child prvky.
        /// Pozor, jednotlivé controly zde mají null, najdou si styl ve svém parentu. Tato proměnná se běžně nepoužívá.
        /// </summary>
        private DfTemplateLayoutStyle __Style;
        /// <summary>
        /// Prvky mých Childs. Výchozí stav je null (většina prvků jsou Controly, které nemají Childs). Lze testovat <see cref="_HasChilds"/>.
        /// </summary>
        private List<DfTemplateLayoutItem> __Childs;
        /// <summary>
        /// Root prvek. Zde nikdy není null: pokud this je Root, pak zde je this.
        /// </summary>
        private DfTemplateLayoutItem _Root { get { return (__Parent?._Root ?? this); } }
        /// <summary>
        /// Styl pro Child prvky tohoto containeru. Pokud this je container, pak má definován svůj vlastní styl (<see cref="__Style"/>).
        /// Pokud jej nemá (typicky proto, že jde o Control), pak zde vrací aktuální styl ze svého Parenta.
        /// </summary>
        private DfTemplateLayoutStyle _CurrentStyle { get { return __Style ?? __Parent?._CurrentStyle; } }
        /// <summary>
        /// Vrátí <see cref="LabelPositionType"/> pro this item, načtený ze stylu <see cref="_CurrentStyle"/>.
        /// Pokud this je Container, pak vrací <see cref="DfTemplateLayoutStyle.CurrentAutoLabelPosition"/>, to se týká labelu grupy.
        /// Pokud this je Control, pak vrací <see cref="DfTemplateLayoutStyle.ChildsAutoLabelPosition"/>, to se týká labelu child prvků v daném containeru.
        /// </summary>
        private LabelPositionType _CurrentAutoLabelPosition { get { return (_IsControl ? _CurrentStyle?.ChildsAutoLabelPosition : _CurrentStyle?.CurrentAutoLabelPosition) ?? LabelPositionType.None; } }
        /// <summary>
        ...
        /// </summary>
        private DfTemplateLayoutItem[] _Childs { get { return __Childs?.ToArray(); } }
        /// <summary>
        ...
        /// </summary>
        private DfTemplateLayoutItem[] _AllChildItems
        {
            get
            {
                List<DfTemplateLayoutItem> allChilds = new List<DfTemplateLayoutItem>();
        ...
        /// </summary>
        private DfTemplateLayoutItem[] _AllChildControls
        {
            get
            {
                List<DfTemplateLayoutItem> allChilds = new List<DfTemplateLayoutItem>();
        ...
        private static void _AddAllChilds(DfTemplateLayoutItem container, List<DfTemplateLayoutItem> allChildsTarget, bool addContainersItem)
        {
        ...
        private void _InitDataPanel(DfPanel dfPanel)
        {
            ...
            this.__ControlInfo = new ControlInfo() { Exists = true, ControlType = DataControlType.Panel, DesignWidth = this.__DesignBounds?.Width, DesignHeight = this.__DesignBounds?.Height };

            _RefreshLabelControlData();
        }
        /// <summary>
        ...
        private void _InitDataGroup(DfGroup dfGroup)
        {
            ...
            this.__ControlInfo = new ControlInfo() { Exists = true, ControlType = DataControlType.Group, DesignWidth = this.__DesignBounds?.Width, DesignHeight = this.__DesignBounds?.Height };
        ...
        private void _InitDataControl(DfBaseControl dfControl)
        {
            ...
            this.__ToolTipVisibility = ToolTipVisibilityType.Default;
            this.__ToolTipTitle = dfControl.ToolTipTitle;
            this.__ToolTipText = dfControl.ToolTipText;

            this.__ControlInfo = new ControlInfo() { Exists = true, ControlType = dfControl.ControlType, Style = dfControl.ControlStyle, DesignWidth = this.__DesignBounds?.Width, DesignHeight = this.__DesignBounds?.Height };

            // Specifické informace podle druhu controlu:
            if (dfControl is DfBaseInputTextControl inputTextControl)
            {   // Text uvnitř Controlu:
                this.__ControlInfo.Text = inputTextControl.Text;
                this.__ControlInfo.IconName = inputTextControl.IconName;
            }
        ...
        /// <summary>
        /// Refreshuje hodnoty <see cref="__MainLabelInfo"/>, <see cref="__ControlInfo"/>, <see cref="__SuffixLabelInfo"/> (jejich Exists a Alignment).<br/>
        /// Nevytváří new instance, ale do existujících instancí ukládá validní údaje.<br/>
        /// Správně vloží objekty pro labely do konkrétních pozice: <see cref="__LabelLeftInfo"/>, <see cref="__LabelTopInfo"/>, <see cref="__LabelRightInfo"/>, <see cref="__LabelBottomInfo"/>.<br/>
        /// Negeneruje do nich new instance, ale umístí do nich referenci na <see cref="__MainLabelInfo"/> a <see cref="__SuffixLabelInfo"/>, podle umístění labelu a jeho existence.
        /// </summary>
        private void _RefreshLabelControlData()
        {
            // Logické rozmístění labelu Main a Suffix:
            LabelPositionType lPos = __LabelPosition ?? _CurrentAutoLabelPosition;
            this.__ValidLabelPosition = lPos;

            var mainLabel = this.__MainLabelInfo;
            bool mainLabelExists = (mainLabel != null && !String.IsNullOrEmpty(mainLabel.Text) && (lPos == LabelPositionType.BeforeLeft || lPos == LabelPositionType.BeforeRight || lPos == LabelPositionType.Top || lPos == LabelPositionType.Bottom || lPos == LabelPositionType.BeforeRight));

            var control = this.__ControlInfo;
            var containerType = this.__ContainerType;
            var controlType = control?.ControlType ?? DataControlType.None;
            bool controlExists = (containerType == ContainerType.Panel || containerType == ContainerType.Group) || !(controlType == DataControlType.None || controlType == DataControlType.PlaceHolder);

            var suffixLabel = this.__SuffixLabelInfo;
            bool suffixLabelExists = (suffixLabel != null && !String.IsNullOrEmpty(suffixLabel.Text) && lPos != LabelPositionType.After);

            if (mainLabel != null)
            {
                mainLabel.Exists = mainLabelExists;
                mainLabel.Alignment = (lPos == LabelPositionType.BeforeLeft ? ContentAlignmentType.MiddleLeft :
                                      (lPos == LabelPositionType.BeforeRight ? ContentAlignmentType.MiddleRight :
                                      (lPos == LabelPositionType.Top ? ContentAlignmentType.BottomLeft :
                                      (lPos == LabelPositionType.After ? ContentAlignmentType.MiddleLeft :
                                      (lPos == LabelPositionType.Bottom ? ContentAlignmentType.TopLeft : ContentAlignmentType.MiddleRight)))));
            }
            if (control != null)
            {
                control.Exists = controlExists;
            }
            if (suffixLabel != null)
            {
                suffixLabel.Exists = suffixLabelExists;
                suffixLabel.Alignment = ContentAlignmentType.MiddleLeft;
            }

            // Fyzické obsazení prostoru pro Labely: Left, Top, Right, Bottom:
            this.__LabelLeftInfo = ((lPos == LabelPositionType.BeforeLeft || lPos == LabelPositionType.BeforeRight) && mainLabelExists ? mainLabel : null);
            this.__LabelTopInfo = ((lPos == LabelPositionType.Top) && mainLabelExists ? mainLabel : null);
            this.__LabelBottomInfo = ((lPos == LabelPositionType.Bottom) && mainLabelExists ? mainLabel : null);
            this.__LabelRightInfo = ((lPos == LabelPositionType.After) && mainLabelExists ? mainLabel : (suffixLabelExists ? suffixLabel : null));
        }
        /// <summary>
        /// Zahodí veškerá data
        /// </summary>
        private void _ResetData()
        {
            ...
            __ContainerType = ContainerType.None;
            __LabelPosition = null;
            __ToolTipVisibility = ToolTipVisibilityType.None;
            __ToolTipTitle = null;
            __ToolTipText = null;
            __ControlInfo = null;
        ...
        /// </summary>
        private string _Key { get { return DfTemplateLayout.GetItemKey(__Name); } }
        /// <summary>
        ...
        /// <summary>
        /// Informace o Controlu (text, velikosti)
        /// </summary>
        private ControlInfo __ControlInfo;
        /// <summary>
        ...
        /// <summary>
        /// Označení prostoru, do kterého byly do this buňky přeneseny labely z layoutu Child prvků (pro jeho krajní labely).
        /// </summary>
        private DfLabelUsedAreaType __ChildExpandAreas;
        ...
        ControlInfo IDataFormItem.ControlInfo { get { return __ControlInfo; } }
        ...
        private void _ResetFlowFinalResults()
        {
            ...
            __ControlInfo?.ResetFlowFinalResults();
        ...
        /// <summary>
        /// Obsahuje prostory, které v tomto Itemu obsazují Labely (je tam instance Label, a má Exists = true)
        /// </summary>
        private DfLabelUsedAreaType _LabelUsedAreas
        {
            get
            {
                return getOneUsedLabelArea(this.__LabelLeftInfo, DfLabelUsedAreaType.Left) |
                       getOneUsedLabelArea(this.__LabelTopInfo, DfLabelUsedAreaType.Top) |
                       getOneUsedLabelArea(this.__LabelRightInfo, DfLabelUsedAreaType.Right) |
                       getOneUsedLabelArea(this.__LabelBottomInfo, DfLabelUsedAreaType.Bottom);

                // Pokud daný LabelInfo je zadán a existuje, pak vrátí daný typ UsedArea
                DfLabelUsedAreaType getOneUsedLabelArea(LabelInfo labelInfo, DfLabelUsedAreaType area)
                {
                    return (labelInfo != null && labelInfo.Exists) ? area : DfLabelUsedAreaType.None;
                }
            }
        }
        /// <summary>
        ...
        bool IFlowLayoutItem.IsContainer { get { return (__ContainerType != ContainerType.None); } }
        ...
        DfLabelUsedAreaType IFlowLayoutItem.LabelUsedAreas { get { return _LabelUsedAreas; } }
        ...
        ControlInfo IFlowLayoutItem.ControlInfo { get { return __ControlInfo; } }
        ...
        Margins IFlowLayoutItem.LabelPlaceholders { get { return __LabelPlaceholders; } }
        ...
        void IFlowLayoutItem.InnerBoundsProcessed() { this._InnerBoundsProcessed(); }
        ...
        internal void ProcessPanel()
        {
            ...
            if (args.LogTime) DxComponent.LogAddLineTime(LogActivityKind.DataFormRepository, $"ProcessLayout panel '{__Name}': {DxComponent.LogTokenTimeMicrosec}", startTime);
            if (args.CreateDebugImages != DfTemplateLayoutDebugImageMode.None) _CreateDebugImage();// Výsledný obrázek?
        }
        /// <summary>
        /// Zajistí plné zpracování this containeru, rekurzivně jeho Child containerů a zdejších Controlů.
        /// </summary>
        private void _ProcessContainer()
        {
            ...
            _ProcessContentSize();                         // Zjištěnou sumární velikost svých Child prvků vhodně uloží do své vlastní velikosti, kde následně slouží pro zpracování Parent containeru, kde my jsme Child
        }
        /// <summary>
        ...
        private void _PositionChilds()
        {
            ...
            List<DfTemplateLayoutItem> processItems = null;                            // Prvky pro další zpracování layoutu (Non Flow)
            ...
            void processFlowItems(List<DfTemplateLayoutItem> items, Size contentSize, out List<DfTemplateLayoutItem> otherItems)
            {
                otherItems = null;

                using (var flowLayout = new DfFlowLayoutInfo(__Style, this.__DfItem.Name))
                {
                    flowLayout.Reset();
                    foreach (var item in items)
                    {
                        ...
                        {   // Prvek jiného typu new Flow odložím do seznamu ostatních prvků, a zpracují se poté:
                            if (otherItems is null) otherItems = new List<DfTemplateLayoutItem>();
                            otherItems.Add(item);
                        }
                    }
                    flowLayout.ProcessFlowItems();
                    if (_LayoutArgs.DebugImagesInfoLevel.HasFlag(DfTemplateLayoutDebugInfoLevelType.AddGuideLines)) __FlowGuideLines = flowLayout.CreateGuideLines();
            ...
            void processNonFlowItems(List<DfTemplateLayoutItem> processItems, Size contentSize, List<DfTemplateLayoutItem> allItems)
            {
                ...
                Dictionary<string, DfTemplateLayoutItem> allDictionary = null;
                ...
                {   // Pokud nějaký prvek bude potřebovat najít svého parenta, tak si připravím Dictionary:
                    allDictionary = new Dictionary<string, DfTemplateLayoutItem>();
            ...
            void processFlowInParentItem(DfTemplateLayoutItem processItem, Size contentSize, Dictionary<string, DfTemplateLayoutItem> allDictionary)
            {
                ...
                if (key != null && allDictionary.TryGetValue(key, out var parentItem) && parentItem.__CellMatrix != null)
                {
                    ...
                    processItem.__ControlInfo.Exists = true;
            ...
            void processBoundsInParentItem(DfTemplateLayoutItem processItem, Size contentSize, Dictionary<string, DfTemplateLayoutItem> allDictionary)
            {
                ...
                {
                    var labelUsedAreas = processItem._LabelUsedAreas;
                    var parentBounds = parentItem.__CellMatrix.GetControlBounds(processItem.__ExpandControl, labelUsedAreas, __Style.TopLabelOffsetX, __Style.BottomLabelOffsetX);  // Souřadnice Controlu v Parent buňce, volitelně Expanded
                    ...
                    DfFlowLayoutInfo.ProcessFixedItemBounds(processItem, controlBounds, parentItem.__CellMatrix.ControlFirstRowHeight, __Style);
                    addItemBoundsToSize(processItem, contentSize);
                }
            }
            ...
            void processFixedAbsoluteItem(DfTemplateLayoutItem processItem, Size contentSize, Dictionary<string, DfTemplateLayoutItem> allDictionary)
            {
                var bounds = processItem.__DesignBounds;
                if (bounds != null)
                {
                    ...
                    DfFlowLayoutInfo.ProcessFixedItemBounds(processItem, controlBounds, null, __Style);
                    addItemBoundsToSize(processItem, contentSize);
                }
            }
            // Přidá souřadnice aktivních oblastí daného prvku do sumární velikosti 'contentSize' (řeší Right a Bottom)
            void addItemBoundsToSize(DfTemplateLayoutItem processItem, Size contentSize)
            {
                addBoundsToSize(processItem.__MainLabelInfo?.Bounds, contentSize);
                addBoundsToSize(processItem.__ControlInfo?.Bounds, contentSize);
                addBoundsToSize(processItem.__SuffixLabelInfo?.Bounds, contentSize);
            }
        ...
        private void _ProcessMargins()
        {
            ...
            FlowGuideLine[] guideLines = __FlowGuideLines;
            ...
            void processMargins(DfTemplateLayoutItem item)
            {
                processBounds(item.__ControlInfo?.Bounds);
                processBounds(item.__MainLabelInfo?.Bounds);
                processBounds(item.__SuffixLabelInfo?.Bounds);
            }
        ...
        /// <summary>
        /// Zjištěnou sumární velikost svých Child prvků vhodně uloží do své vlastní velikosti, kde následně slouží pro zpracování Parent containeru, kde my jsme Child.<br/>
        /// Vychází jednak z velikosti <see cref="__ContentSize"/> (prostá velikost obsahu), a druhak z <see cref="__FlowLayoutMatrix"/> (strukturovaná velikost jednotlivých oblastí).
        /// Výsledkem jsou velikosti ukládané do <see cref="__ControlInfo"/>...
        ...
        {
            var contentSize = this.__ContentSize;                    // Prostá vnitřní velikost
            var flowMatrix = this.__FlowLayoutMatrix;                // Vnitřní souřadnice FlowLayoutu = dané mými Childs prvky = jejich Controly a vnější labely

            // Musím ještě vyřešit souběh FlowLayout a BoundsLayout, kdy 

            if (this._IsRoot || flowMatrix is null)
            {   // Jednoducho:
                this.__ControlInfo.ImplicitOptimalWidth = contentSize?.Width;
                this.__ControlInfo.ImplicitOptimalHeight = contentSize?.Height;
            }
            else
            {   // Máme FlowLayout i parenta (který bude počítat rozmístění svých Child prvků = tedy i nás).
                // Nyní chceme, aby se správně koordinovaly prostory pro Labely moje (this = Item může mít svůj Label) plus labely mých Child prvků.
                // Například this prvek nebude mít label na pozici Top, ale máme Child prvky, které si právě vytvořily svůj prostor (__FlowLayoutMatrix => flowMatrix),
                //   a v něm mají nahoře nějaké labely => a my nyní chceme, aby tyto Child labely umístěné Top byly ve společném pruhu se sousedními buňkami, které mohou být jednoduché (ne vnořené grupy) a mají svůj label umístěný Top.
                //   (Jiná situace by byla, když bych já jako Item měl svůj Top label, pak jej zobrazím, a Child prvky budou mít label níže).
                // Samozřejmě chci, aby potom i výška Controlu v Child prvcích odpovídala jen controlům, už bez labelu. A navazovala tak na vedlejší Itemy.
                // Tedy: pokud Child prvky mají ve svém layoutu prostor pro krajní Labely (Left, Top, Right, Bottom) a my jako Item na dané straně Label nemáme, pak ten prostor přenecháme našemu Child layoutu pro jeho Labely na dané straně.
                //  1. Velikost využitého prostoru z Child layoutu přeneseme do Placeholderu = čtyři dimenze, určující potřebný prostor okolo Controlu;
                //  2. Snížíme tím potřebnou velikost pro náš Control;
                //  3. Parent prvek pak počítá Layout, akceptuje prostory Placeholder na těch stranách, kde nemá Layout (Left, Top, ...)
                //  4. Parent prvek po dopočtení Layoutu pak ukládá vypočtené souřadnice, pokud najde LabelInfo na určité straně, pak do něj vloží Bounds; pokud najde ControlInfo pak volá jej, obecně volá ...
                _RefreshLabelControlData();                                    // Validně obsadí pozice labelů Left, Top, Right, Bottom
                ...
                prepareLabelMargin(DfLabelUsedAreaType.Left, isCombineLabelAreas, cellLabelUsedAreas, childLabelUserAreas, flowMatrix, controlSize, labelPlaceholders, ref childExpandAreas);
                prepareLabelMargin(DfLabelUsedAreaType.Top, isCombineLabelAreas, cellLabelUsedAreas, childLabelUserAreas, flowMatrix, controlSize, labelPlaceholders, ref childExpandAreas);
                prepareLabelMargin(DfLabelUsedAreaType.Right, isCombineLabelAreas, cellLabelUsedAreas, childLabelUserAreas, flowMatrix, controlSize, labelPlaceholders, ref childExpandAreas);
                prepareLabelMargin(DfLabelUsedAreaType.Bottom, isCombineLabelAreas, cellLabelUsedAreas, childLabelUserAreas, flowMatrix, controlSize, labelPlaceholders, ref childExpandAreas);
            ...
            void prepareLabelMargin(DfLabelUsedAreaType currArea, bool isCombineAreas, DfLabelUsedAreaType cellAreas, DfLabelUsedAreaType childAreas, CellMatrixInfo flowMatrix, Size ctlSize, Margins lblPlaceholders, ref DfLabelUsedAreaType expandAreas)
            {
                if (!childAreas.HasFlag(currArea)) return;                     // Pokud Child prvky nepoužívají danou stranu (Top, Left) pro svoje labely, pak daný prostor nebudu používat ani pro ně
                ...
                switch (currArea)
                {
                    case DfLabelUsedAreaType.Left:
                        expandAreas |= currArea;
                        lblPlaceholders.Left = flowMatrix.LeftLabelWidth;
                        ctlSize.Width -= flowMatrix.ControlLeft;
                        break;
                    case DfLabelUsedAreaType.Top:
                        expandAreas |= currArea;
                        lblPlaceholders.Top = flowMatrix.TopLabelHeight;
                        ctlSize.Height -= flowMatrix.ControlTop;
                        break;
                    case DfLabelUsedAreaType.Right:
                        expandAreas |= currArea;
                        lblPlaceholders.Right = flowMatrix.RightLabelWidth;
                        ctlSize.Width -= (flowMatrix.CellRight - flowMatrix.ControlRight);
                        break;
                    case DfLabelUsedAreaType.Bottom:
                        expandAreas |= currArea;
                        lblPlaceholders.Bottom = flowMatrix.BottomLabelHeight;
                        ctlSize.Height -= (flowMatrix.CellBottom - flowMatrix.ControlBottom);
                        break;
                }
            }
        }
        /// <summary>
        /// Volá se pouze pro Containery (jejichž je <see cref="IFlowLayoutItem.IsContainer"/> true), volá se po vložení souřadnic do Bounds do <see cref="__ControlInfo"/> a do jednotlivých Labelů.
        /// V tuto chvíli je platný obsah <see cref="__CellMatrix"/>.
        /// Container může na základě těchto hodnot modifikovat souřadnice svých Child prvků.
        /// </summary>
        private void _InnerBoundsProcessed()
        {
            // Situace: Do this.__ControlInfo.Bounds  byla právě vložena souřadnice obsahu controlu, zmenšená o ty vnější Labely z našich Child prvků,
            //   které jsme ve zdejší metodě _ProcessContentSize() "přesunuli" na místo Labelů zdejšího prvku (protože zdejší prvek je nepoužívá).
            // Přesunutí vidíme v hodnotách this.__LabelPlaceholders: pokud je některá hodnota kladná, pak Child labely na dané straně jsme umístili namísto našich labelů, a zmenšili jsme v daném směru zdejší control.

            // Reálný příklad:
            //  Máme prvek, který nemá žádný label, nachází se na 1. řádku celého panelu, obsahuje grupu se 4 prvky v jednom řádku, labely umístěné Top.
            //  Pak jeho  __CellMatrix = { Left: 0; Right: 309; Top: 0; Bottom: 38}, TopLabelTop = 0, TopLabelHeight = 16, ControlTop = 18, ControlHeight = 20, ControlBottom = 38
            //  Ale protože obsah grupy je zvenku chápán jako Control, tak po určení dimenzí je vepsána souřadnice:
            //    __ControlInfo.Bounds = {Left: 0; Top: 18; Width: 300; Height: 20}	
            //  Hodnoty v __LabelPlaceholders = {Left: 0; Top: 16; Right: 0; Bottom: 0}	= rezervace prostoru pro labely (výška labelu Top = 16px, je čerpána z __CellMatrix.TopLabelHeight = 16,
            //    zajistila posunutí __ControlInfo.Bounds.Top = 18 = 16px Label + 2px Margin !!! 
            var controlBounds = this.__ControlInfo.Bounds;
            if (controlBounds is null) return;

            var lblPlaceholders = this.__LabelPlaceholders;
            if (lblPlaceholders.IsEmpty) return;

            var cellMatrix = this.__CellMatrix;
            var flowMatrix = this.__FlowLayoutMatrix;

            int fx = controlBounds.Left;
            int fy = controlBounds.Top;
            int fw = controlBounds.Width;
            int fh = controlBounds.Height;

            if (lblPlaceholders.Left > 0)
            {   // Naše Left labely jsme přesunuli doleva do prostoru LabelLeft = nyní přemístíme celý prostor controlBounds doleva tak, aby náš prostor Control.Left odpovídal okolnímu ControlLeft = controlBounds.Left:
                int dx = flowMatrix.ControlLeft - flowMatrix.LeftLabelLeft;
                fx = controlBounds.Left - dx;
                fw += dx;
            }
            if (lblPlaceholders.Top > 0)
            {   // Naše Top labely jsme přesunuli nahoru do prostoru LabelTop = nyní přemístíme celý prostor controlBounds nahoru tak, aby náš prostor Control.Top odpovídal okolnímu ControlTop = controlBounds.Top:
                int dy = flowMatrix.ControlTop - flowMatrix.TopLabelTop;
                fy = controlBounds.Top - dy;
                fh += dy;
            }
            if (lblPlaceholders.Right > 0)
            {   // Naše Right labely jsme přesunuli doprava do prostoru LabelRight = nyní rozšíříme celý prostor controlBounds doprava tak, aby náš prostor Control.Right obsahoval i labely Right:
                int dw = flowMatrix.RightLabelRight - flowMatrix.ControlRight;
                fw += dw;
            }
            if (lblPlaceholders.Bottom > 0)
            {   // Naše Bottom labely jsme přesunuli dolů do prostoru LabelBottom = nyní rozšíříme celý prostor controlBounds dolů tak, aby náš prostor Control.Bottom obsahoval i labely Bottom:
                int dh = flowMatrix.BottomLabelBottom - flowMatrix.ControlBottom;
                fh += dh;
            }

            controlBounds.Left = fx;
            controlBounds.Top = fy;
            controlBounds.Width = fw;
            controlBounds.Height = fh;
        }
        /// <summary>
        ...
        /// <summary>
        /// Vodící linky vytvářené pro režim FlowLayout, pokud v argumentu <see cref="_LayoutArgs"/> je požadováno <see cref="DfTemplateLayoutArgs.DebugImagesInfoLevel"/> = true.
        /// </summary>
        ...
        private static void _PrepareContainerAbsoluteBounds(DfTemplateLayoutItem container, int addX, int addY)
        {
            var childs = container.__Childs;
            if (childs != null)
            {
                foreach (var child in childs)
                {
                    if (child != null)
                    {
                        ...
                        if (child._HasChilds)
                        {
                            int nextX = child.__ControlInfo.Bounds.Left;
                            int nextY = child.__ControlInfo.Bounds.Top;
            ...
            void prepareAbsoluteBounds(DfTemplateLayoutItem item, int dx, int dy)
            {
                if (dx != 0 || dy != 0)
                {
                    prepareAbsoluteBoundsOne(item.__ControlInfo?.Bounds, dx, dy);
                    prepareAbsoluteBoundsOne(item.__MainLabelInfo?.Bounds, dx, dy);      // Panel nemá MainLabel
                    prepareAbsoluteBoundsOne(item.__SuffixLabelInfo?.Bounds, dx, dy);    // Grupa sice má MainLabel, ale nemá SuffixLabel
                }
            }
        ...
        /// </summary>
        /// <returns></returns>
        private void _CreateDebugImage()
        {
            var startTime = DxComponent.LogTimeCurrent;

            var args = this._LayoutArgs;
            switch (args.CreateDebugImages)
            {
                case DfTemplateLayoutDebugImageMode.File:
                    createDebugImageFile();
                    break;
                case DfTemplateLayoutDebugImageMode.Image:
                    createDebugImageItem();
                    break;
            }
            ...
            // Vytvoří Image a uloží jej do patřičného souboru
            void createDebugImageFile()
            {
                var fileName = getImageName(true, true, ".png");
                ...
                using (var image = _CreateImage())
                {
                    if (image != null)
                    {
                        ...
                        if (args.ImageFiles is null) args.ImageFiles = new List<string>();
                        args.ImageFiles.Add(fullFile);

                    }
                }
            }
            // Vytvoří Image a přidá jej do pole
            void createDebugImageItem()
            {
                var fileName = getImageName(false, false, null);
                var image = _CreateImage();

                if (args.Images is null) args.Images = new List<Tuple<string, System.Drawing.Image>>();
                args.Images.Add(new Tuple<string, System.Drawing.Image>(fileName, image));
            }
            // Vytvoří a vrátí jméno pro soubor
            string getImageName(bool addFile, bool addTimeId, string extension)
            {
                string n = "";
                if (addFile)
                {   // Form Filename:
                ...
                if (addTimeId)
                {
                    ...
                    long msec = now.Millisecond +
                        1000 * now.Second +
                        60000 * now.Minute +
                        3600000 * now.Hour;
                    long days = now.DayOfYear + 368 * (now.Year % 12);
                    long time = 86400000L * days + msec;
                    n += "_" + time.ToString();
                }
        ...
        private System.Drawing.Image _CreateImage()
        {
            ...
            var infoLevel = _LayoutArgs.DebugImagesInfoLevel;
            var drawLabelBackground = infoLevel.HasFlag(DfTemplateLayoutDebugInfoLevelType.AddLabelBackground);
            var drawLabelBorder = infoLevel.HasFlag(DfTemplateLayoutDebugInfoLevelType.AddLabelBorder);
            var drawLabelDebug = drawLabelBackground || drawLabelBorder;
            ...
            var controlLabelBorderColor = System.Drawing.Color.FromArgb(255, 140, 140, 140);
            ...
            using (var stringFormat = new System.Drawing.StringFormat(System.Drawing.StringFormatFlags.NoWrap | System.Drawing.StringFormatFlags.LineLimit))
            {
                ...
                if (drawGuideLines)
                {
                    var guideLines = __FlowGuideLines;
                    if (guideLines != null)
                    {
            ...
            void drawItem(DfTemplateLayoutItem item, System.Drawing.Graphics graphics, System.Drawing.SolidBrush brush, System.Drawing.Pen pen, System.Drawing.StringFormat stringFormat)
            {
                var iFlow = item as IFlowLayoutItem;
                var iData = item as IDataFormItem;

                // Labely:
                drawLabelInfo(graphics, brush, pen, stringFormat, iFlow.LabelLeftInfo, ContentAlignmentType.MiddleLeft);
                drawLabelInfo(graphics, brush, pen, stringFormat, iFlow.LabelTopInfo, ContentAlignmentType.MiddleLeft);
                drawLabelInfo(graphics, brush, pen, stringFormat, iFlow.LabelRightInfo, ContentAlignmentType.MiddleLeft);
                drawLabelInfo(graphics, brush, pen, stringFormat, iFlow.LabelBottomInfo, ContentAlignmentType.MiddleLeft);

                // Control (nikoliv Container):
                var controlInfo = iFlow.ControlInfo;
                if (item._IsControl && controlInfo != null && controlInfo.Exists && controlInfo.Bounds != null)
                {
                    switch (item._DfControl.ControlType)
                    {
                        case DataControlType.Label:
                            ContentAlignmentType labelAlignment = ContentAlignmentType.MiddleLeft;
                            string text = iData.Name;
                            if (item.__DfItem is DfLabel dfLabel)
                            {
                                if (dfLabel.Alignment.HasValue) labelAlignment = dfLabel.Alignment.Value;
                                text = dfLabel.Text;
                            }
                            if (drawLabelBorder)
                                drawControlBorder(graphics, brush, pen, controlInfo.Bounds, null, controlLabelBorderColor);
                            drawControlLabel(graphics, brush, pen, stringFormat, text, controlInfo.Bounds, labelAlignment);
                            break;
                        case DataControlType.HLine:
                            int hLineWidth = 2;
                            if (item.__DfItem is DfHLine dfHLine && dfHLine.LineWidth.HasValue)
                                hLineWidth = dfHLine.LineWidth.Value;
                            drawControlHLine(graphics, brush, pen, stringFormat, iData.Name, controlInfo.Bounds, ContentAlignmentType.MiddleCenter, hLineWidth);
                            break;
                        case DataControlType.VLine:
                            int vLineWidth = 2;
                            if (item.__DfItem is DfVLine dfVLine && dfVLine.LineWidth.HasValue)
                                vLineWidth = dfVLine.LineWidth.Value;
                            drawControlVLine(graphics, brush, pen, stringFormat, iData.Name, controlInfo.Bounds, ContentAlignmentType.MiddleCenter, vLineWidth);
                            break;
                        default:
                            drawControlBorder(graphics, brush, pen, controlInfo.Bounds, controlBackColor, controlBorderColor);
                            drawControlName(graphics, brush, pen, stringFormat, iData.Name, controlInfo.Bounds, ContentAlignmentType.MiddleCenter);
                            break;
                    }
                }
            }
            // Vykreslí LabelInfo včetně podkladu
            void drawLabelInfo(System.Drawing.Graphics graphics, System.Drawing.SolidBrush brush, System.Drawing.Pen pen, System.Drawing.StringFormat stringFormat, LabelInfo labelInfo, ContentAlignmentType alignment)
            {
                if (labelInfo is null || !labelInfo.Exists || labelInfo.Bounds is null) return;
                drawLabel(graphics, brush, pen, stringFormat, labelInfo.Text, labelInfo.Bounds, labelInfo.Alignment ?? alignment);
            }
            ...
            void drawControlHLine(System.Drawing.Graphics graphics, System.Drawing.SolidBrush brush, System.Drawing.Pen pen, System.Drawing.StringFormat stringFormat, string text, ControlBounds bounds, ContentAlignmentType alignment, int lineWidth)
            {
                if (lineWidth <= 0) return;
                var r = getRectangle(bounds);
                r.Y = r.Top + ((r.Height - lineWidth) / 2);
                r.Height = lineWidth;
                graphicsForRectangles(graphics);
            ...
            void drawControlVLine(System.Drawing.Graphics graphics, System.Drawing.SolidBrush brush, System.Drawing.Pen pen, System.Drawing.StringFormat stringFormat, string text, ControlBounds bounds, ContentAlignmentType alignment, int lineWidth)
            {
                var r = getRectangle(bounds);
                r.X = r.Left + ((r.Width - lineWidth) / 2);
                r.Width = lineWidth;
                graphicsForRectangles(graphics);
                brush.Color = controlLineColor;
                graphics.FillRectangle(brush, r);
            }
            // Vykreslí Control typu Label, nikoli jeho podklad
            ...
            // Vykreslí Border a podklad pro Control
            void drawControlBorder(System.Drawing.Graphics graphics, System.Drawing.SolidBrush brush, System.Drawing.Pen pen, ControlBounds bounds, System.Drawing.Color? fillColor, System.Drawing.Color? borderColor)
            {
                var r = getRectangle(bounds);
            ...
            // Vykreslí Name do Controlu
            void drawControlName(System.Drawing.Graphics graphics, System.Drawing.SolidBrush brush, System.Drawing.Pen pen, System.Drawing.StringFormat stringFormat, string text, ControlBounds bounds, ContentAlignmentType alignment)
            {
                var r = getRectangle(bounds);
                if (isValidBounds(bounds))
                {
    ...
    internal class DfFlowLayoutInfo : IDisposable
    {
        ...
        public DfFlowLayoutInfo(DfTemplateLayoutStyle layoutStyle, string sourceId)
        {
        ...
        {
            DfTemplateLayoutStyle layoutStyle = __Style;
            // Nějak dané šířky:
            if (layoutStyle != null)
        ...
        private void _LoadStyle()
        {
            ...
            __AutoLabelPosition = style.ChildsAutoLabelPosition;
        ...
        public void AddFlowItem(IFlowLayoutItem layoutItem)
        {
            ...
            void storeResults(int left, int right, int top, int bottom)
            {
                ...
                {   // Pomalý cyklus jde přes řádky:
                    ...
                    __Rows[r].ContainsItem = true;                   // Řádek obsahuje nějaký prvek (informace pro pozdější nápočet souřadnice)
                    var rowCells = __Cells[r];                       // Obsah daného existujícího řádku = pochází z 2D matice buněk: __Cells
                    for (int c = left; c < right; c++)
                    {   // Rychlý cyklus jde přes sloupce:
                        if (rowCells[c] != null)
                            throw new InvalidOperationException($"FlowLayout: pokus o vícenásobné použití buňky [Row:{r},Col:{c}].");
                        rowCells[c] = layoutItem;                    // Danou buňku obsadíme daným prvkem
                        __Columns[c].ContainsItem = true;            // Sloupec obsahuje nějaký prvek (informace pro pozdější nápočet souřadnice)
                    }
                }
        ...
        {
            // Shift doleva pro LabelTop a LabelBottom, který musí rezervovat pixely v prostoru Left:
            int shiftTopLabel = getShiftLabel(this.__TopLabelOffsetX);
            int shiftBottomLabel = getShiftLabel(this.__BottomLabelOffsetX);

            // Všechny prvky zpracují svoje labely:
            __Items.ForEach(i => processLabels(i));


            // Vrátí Shift doleva pro LabelTop a LabelBottom, který musí rezervovat pixely v prostoru Left:
            int getShiftLabel(int offsetX)
            {
                ...
                int shift = (offsetX < 0 ? -offsetX : 0);            // Pokud je offset záporný, pak jde o shift doleva = ten bude kladný:
                int margin = this.__ControlMargins?.Left ?? 0;       // Mezera mezi labelem vlevo a controlem
                if (shift > 0 && margin > 0)
            ...
            void processLabels(IFlowLayoutItem item)
            {
                var placeHolders = item.LabelPlaceholders;
                var controlMargins = this.__ControlMargins;
                processLabel(item.LabelLeftInfo, placeHolders?.Left, controlMargins?.Left, __Columns, item.FlowColBeginIndex, true, true);
                processLabel(item.LabelTopInfo, placeHolders?.Top, controlMargins?.Top, __Rows, item.FlowRowBeginIndex, false, true);
                processLabel(item.LabelRightInfo, placeHolders?.Right, controlMargins?.Right, __Columns, item.FlowColEndIndex, true, false);
                processLabel(item.LabelBottomInfo, placeHolders?.Bottom, controlMargins?.Bottom, __Rows, item.FlowRowEndIndex, false, false);

                processLabelShift(item.LabelTopInfo, __Columns, item.FlowColBeginIndex, shiftTopLabel);
                processLabelShift(item.LabelBottomInfo, __Columns, item.FlowColBeginIndex, shiftBottomLabel);
            }
            // Zpracuje jeden label (do sloupce / do řádku, v dané velikosti, rozměr Width / Height, do prostoru Before / After)
            void processLabel(LabelInfo labelInfo, int? placeHolder, int? space, List<LineInfo> lines, int? lineIndex, bool isWidth, bool isBefore)
            {
                if (!lineIndex.HasValue) return;

                // Máme daný label a ten existuje? { pak řešíme: šířku nebo výšku (isWidth) ? : šířka uznává i DesignWidth jako prioritu. Jinak bereme Measured rozměr)
                // Akceptujeme i dodaný placeholder!

                int labelSize = ((labelInfo != null && labelInfo.Exists) ? ((isWidth ? (labelInfo.DesignWidth ?? labelInfo.MeasuredWidth) : labelInfo.MeasuredHeight) ?? 0) : 0);
                int placeSize = placeHolder ?? 0;
                if (labelSize > 0 || placeSize > 0)
                {
                    // Mezera m se aplikuje tehdy, když máme současně dánu velikost Labelu i Placeholderu:
                    int m = ((labelSize > 0 && placeSize > 0) ? (space ?? 0) : 0);

                    // Určíme velikost Labelu = velikost Labelu + (mezera) + velikost placeholderu:
                    int size = labelSize + m + placeSize;
                    if (size > 0)
                        processLabelSize(lines, lineIndex, size, isBefore);
                }
            }
            // Zpracuje jeden shift labelu (do sloupce, v dané velikosti, do prostoru Before)
            void processLabelShift(LabelInfo labelInfo, List<LineInfo> lines, int? lineIndex, int shift)
            {
                if (labelInfo is null || !labelInfo.Exists || !lineIndex.HasValue) return;

                // Daný Shift zařadíme do sloupce jako prostor LabelBeforeMaximalSize:
                processLabelSize(lines, lineIndex, shift, true);
            }
            // Zpracuje jeden label (do sloupce / do řádku, v dané velikosti, do prostoru Before / After)
            void processLabelSize(List<LineInfo> lines, int? lineIndex, int? size, bool isBefore)
            {
        ...
        private void _ProcessControlSingleSpan()
        {
            ...
            {
                if (item.FlowColBeginIndex == item.FlowColEndIndex && item.FlowColBeginIndex == column.Index && item.ControlInfo != null && item.ControlInfo.Exists)
                {   // Prvek má ColSpan = 1 a to daný column, a obsahuje Control a ten Existuje:
                    var controlInfo = item.ControlInfo;                                  // item = komplet (Labely + Textbox) / controlInfo = pouze ten Textbox
                    bool sizeIsPercent = controlInfo.DesignWidthPercent.HasValue;        // true = je definovaná šířka v procentech => nemá vliv na šířku sloupce!!!
                    if (!sizeIsPercent)
                    {
                        column.ControlBoundsMaximalSize = controlInfo.DesignWidthPixel;
                        column.ControlImplicitMaximalSize = _GetMax(controlInfo.ImplicitMinimalWidth, controlInfo.ImplicitOptimalWidth);
            ...
            {
                if (item.FlowRowBeginIndex == item.FlowRowEndIndex && item.FlowRowBeginIndex == row.Index && item.ControlInfo != null && item.ControlInfo.Exists)
                {
                    var controlInfo = item.ControlInfo;                                  // item = komplet (Labely + Textbox) / controlInfo = pouze ten Textbox
                    bool sizeIsPercent = controlInfo.DesignHeightPercent.HasValue;       // true = je definovaná výška v procentech => nemá vliv na výšku řádku!!!
                    if (!sizeIsPercent)
                    {
                        row.ControlBoundsMaximalSize = controlInfo.DesignHeightPixel;
                        row.ControlImplicitMaximalSize = _GetMax(controlInfo.ImplicitMinimalHeight, controlInfo.ImplicitOptimalHeight);
                        item.AcceptedWidth = true;
                    }
                }
            }
        ...
        private void _ProcessControlMultiSpan()
        {
            ...
            bool processItemWidth(IFlowLayoutItem item, int qWave)
            {
                ...
                int? implicitWidth = _GetMax(controlInfo.ImplicitMinimalWidth, controlInfo.ImplicitOptimalWidth);
                int? designWidthPixel = controlInfo.DesignWidthPixel;
                LineInfo.ControlSizeType sizeType = (designWidthPixel.HasValue ? LineInfo.ControlSizeType.MaxBounds : implicitWidth.HasValue ? LineInfo.ControlSizeType.MaxImplicit : LineInfo.ControlSizeType.None);
                ...
                int sizeItem = designWidthPixel ?? implicitWidth ?? 0;
            ...
            bool processItemHeight(IFlowLayoutItem item, int qWave)
            {
                ...
                // Hodnota sizeType má vliv na to, zda vůbec budu hledat SizeSum (pokud já nemám rozměr Design nebo Implicit, tak hledat nemusím), a kam ji pak ukládat.
                int? implicitHeight = _GetMax(controlInfo.ImplicitMinimalHeight, controlInfo.ImplicitOptimalHeight);
                int? designHeightPixel = controlInfo.DesignHeightPixel;
                LineInfo.ControlSizeType sizeType = (designHeightPixel.HasValue ? LineInfo.ControlSizeType.MaxBounds : implicitHeight.HasValue ? LineInfo.ControlSizeType.MaxImplicit : LineInfo.ControlSizeType.None);
                if (sizeType == LineInfo.ControlSizeType.None) return false;   // Není nutno
                ...
                // Obousměrné zpracování (Columns i Rows):
                int sizeItem = designHeightPixel ?? implicitHeight ?? 0;
            ...
            {
                var controlInfo = item.ControlInfo;                            // item = komplet (Labely + Textbox) / controlInfo = pouze ten Textbox
                var designWidthPixel = controlInfo.DesignWidthPixel;
                return ((designWidthPixel.HasValue || controlInfo.ImplicitMinimalWidth.HasValue || controlInfo.ImplicitOptimalWidth.HasValue) && !item.AcceptedWidth);
            }
            ...
            {
                var controlInfo = item.ControlInfo;                            // item = komplet (Labely + Textbox) / controlInfo = pouze ten Textbox
                var designHeightPixel = controlInfo.DesignHeightPixel;
                return ((designHeightPixel.HasValue || controlInfo.ImplicitMinimalHeight.HasValue || controlInfo.ImplicitOptimalHeight.HasValue) && !item.AcceptedHeight);
            }
        ...
        private void _ProcessItemsBounds()
        {
            ...
            foreach (var i in __Items)
            {
                ...
                _ProcessInnerItemBounds(i, __LabelsRelativeToControl, __TopLabelOffsetX, __BottomLabelOffsetX, margins);
            }
        }
        /// <summary>
        /// Metoda vytvoří a vrátí pole vodících linek <see cref="FlowGuideLine"/>
        ...
        private FlowGuideLine[] _CreateGuideLines()
        {
            ...
            addLines(__Columns);
            addLines(__Rows);
            guideLines.Sort(FlowGuideLine.CompareByPosition);
            return guideLines.ToArray();

            // Přidá GuideLines za jednu osu:
            void addLines(List<LineInfo> lines)
            {
                ...
                {
                    var line = lines[i];
                    // Zcela prázdné dimenze do výstupu nebudu dávat:
                    if (line.CurrentEnd.Value > line.CurrentBegin.Value)
                    {
        ...
        public static void ProcessInnerItemBounds(IFlowLayoutItem item, DfTemplateLayoutStyle layoutStyle)
        {
            ...
            _ProcessInnerItemBounds(item, labelsRelativeToControl, topLabelOffsetX, bottomLabelOffsetX, layoutStyle.Margins);
        }
        /// <summary>
        /// Metoda určí a uloží do prvku <see cref="IFlowLayoutItem"/> <paramref name="item"/> souřadnice pro MainLabel, pro Control a pro SuffixLabel podle standardních pravidel.
        /// Metoda je používána pro běžné prvky FlowLayoutu, a i pro prvky, které mají Fixed layout, ale chtějí se částečně umístit do okolního FlowLayoutu pomocí Parenta = <c>PlaceHolder</c>.
        ...
        public static void ProcessInnerItemBounds(IFlowLayoutItem item, bool labelsRelativeToControl, int topLabelOffsetX, int bottomLabelOffsetX, Margins margins)
        {
            if (item is null) throw new ArgumentNullException($"DfFlowLayoutInfo.ProcessInnerItemBounds fail: item is null.");
            if (item.CellMatrix is null) throw new ArgumentNullException($"DfFlowLayoutInfo.ProcessInnerItemBounds fail: item.CellBounds is null.");

            _ProcessInnerItemBounds(item, labelsRelativeToControl, topLabelOffsetX, bottomLabelOffsetX, margins);
        }
        /// <summary>
        ...
        /// <param name="firstRowHeight">Výška prvního řádku = výška labelu na pozici Left a Right</param>
        /// <param name="layoutStyle">Definice stylu obsahuje v sobě potřebné hodnoty</param>
        public static void ProcessFixedItemBounds(IFlowLayoutItem item, ControlBounds controlBounds, int? firstRowHeight, DfTemplateLayoutStyle layoutStyle)
        {
            ...
            _ProcessFixedItemBounds(item, controlBounds, firstRowHeight, topLabelOffsetX, bottomLabelOffsetX, margins);
        }
        /// <summary>
        /// Metoda určí a uloží do prvku <see cref="IFlowLayoutItem"/> <paramref name="item"/> souřadnice pro MainLabel, pro Control a pro SuffixLabel podle dodané souřadnice pro Control.
        /// </summary>
        ...
        /// <param name="bottomLabelOffsetX">Offset X pro labely umístěné Bottom</param>
        /// <param name="margins">Okraje kolem controlu vzhledem k labelům</param>
        public static void ProcessFixedItemBounds(IFlowLayoutItem item, ControlBounds controlBounds, int? firstRowHeight, int topLabelOffsetX, int bottomLabelOffsetX, Margins margins)
        {
            if (item is null) throw new ArgumentNullException($"DfFlowLayoutInfo.ProcessFixedItemBounds fail: item is null.");
            if (controlBounds is null) throw new ArgumentNullException($"DfFlowLayoutInfo.ProcessFixedItemBounds fail: controlBounds is null.");
            _ProcessFixedItemBounds(item, controlBounds, firstRowHeight, topLabelOffsetX, bottomLabelOffsetX, margins);
        ...
        /// <param name="bottomLabelOffsetX">Offset X pro labely umístěné Bottom</param>
        /// <param name="margins">Okraje kolem controlu vzhledem k labelům</param>
        private static void _ProcessInnerItemBounds(IFlowLayoutItem item, bool labelsRelativeToControl, int topLabelOffsetX, int bottomLabelOffsetX, Margins margins)
        {
            var labelUsedAreas = item.LabelUsedAreas;                                    // Které prostory vedle Controlu jsou obsazeny Labelem?
            var controlBounds = processItemControl(labelUsedAreas);                      // Souřadnice Controlu, možná jsou Extended do nevyužitých prostor místo Labelu, když tam Label není
            var labelPlaceholders = item.LabelPlaceholders;
            
            processItemLabel(item.LabelLeftInfo, DfLabelUsedAreaType.Left);              // Umístí Label do prostoru vlevo před control
            processItemLabel(item.LabelTopInfo, DfLabelUsedAreaType.Top);                //  ... nad control
            processItemLabel(item.LabelRightInfo, DfLabelUsedAreaType.Right);            //  ... vpravo
            processItemLabel(item.LabelBottomInfo, DfLabelUsedAreaType.Bottom);          //  ... dolů
            if (item.IsContainer) item.InnerBoundsProcessed();
            // hotovo.


            // Zpracuje souřadnici pro Control
            ControlBounds processItemControl(DfLabelUsedAreaType labelUsedAreas)
            {
                var controlInfo = item.ControlInfo;
                if (controlInfo is null || !controlInfo.Exists || item.CellMatrix is null) return null;
                ...
                // Souřadnice zvolené oblasti, určené pro umístění Controlu, celý dostupný prostor (nemusí být využit celý),
                //  při akceptování 'itemExpand' a 'labelUsedAreas', včetně X-offsetů pro Labely Top a Bottom:
                var totalBounds = item.CellMatrix.GetControlBounds(itemExpand, labelUsedAreas, topLabelOffsetX, bottomLabelOffsetX);

                // Souřadnice fyzického Controlu, zmenšené na cílový rozměr, zarovnané do vyhrazené oblasti:
                ControlBounds controlBounds = controlInfo.GetControlBounds(totalBounds, item.DesignHPosition, item.DesignVPosition);
                controlInfo.Bounds = controlBounds;
        ...
        /// <param name="bottomLabelOffsetX">Offset X pro labely umístěné Bottom</param>
        /// <param name="margins">Okraje kolem controlu vzhledem k labelům</param>
        private static void _ProcessFixedItemBounds(IFlowLayoutItem item, ControlBounds controlBounds, int? firstRowHeight, int topLabelOffsetX, int bottomLabelOffsetX, Margins margins)
        {
            processControl(item.ControlInfo, controlBounds);
            processLabel(item.LabelLeftInfo, controlBounds, DfLabelUsedAreaType.Left);
            processLabel(item.LabelTopInfo, controlBounds, DfLabelUsedAreaType.Top);
            processLabel(item.LabelRightInfo, controlBounds, DfLabelUsedAreaType.Right);
            processLabel(item.LabelBottomInfo, controlBounds, DfLabelUsedAreaType.Bottom);

            // Uloží bounds do controlu
            void processControl(ControlInfo controlInfo, ControlBounds controlBounds)
            {
                if (controlInfo is null || !controlInfo.Exists) return;
                controlInfo.Bounds = controlBounds;
            }
            // Vypočítá a uloží bounds do labelu, zohlední přitom jeho pozici, určí rozměry labelu, vychází z pozice controlu plus margins + offset
            void processLabel(LabelInfo labelInfo, ControlBounds controlBounds, DfLabelUsedAreaType area)
            {
                if (labelInfo is null || !labelInfo.Exists) return;

                // Rozměry labelu:
                int width = labelInfo.DesignWidth ?? labelInfo.MeasuredWidth ?? 0;
                int heightL = labelInfo.MeasuredHeight ?? 0;                                                            // Výška vlastního textu labelu
                int heightR = (firstRowHeight.HasValue && firstRowHeight.Value > 0 ? firstRowHeight.Value : heightL);   // Výška prvního řádku = výška labelu na pozici Left a Right
                if (width <= 0 || heightL <= 0) area = DfLabelUsedAreaType.None;
        ...
        internal class LineInfo : IDisposable
        {
            ...
            /// Nastavuje se v procesu <see cref="DfFlowLayoutInfo.AddFlowItem(IFlowLayoutItem)"/> = když se prvek umísťuje do řádků a sloupců (prvek si obsazuje jednotlivé buňky).
            ...
            /// Lze nastavit na true, ale nelze vrátit z true na false.<br/>
            /// Vyhodnocuje se při určení <see cref="ControlSize"/>, kdy pro prázdnou dimenzi (která nemá <see cref="ContainsItem"/>) a bez deklarované velikosti se výsledná velikost = 0.
            /// </summary>
            ...
            public int? GetControlSize(ControlSizeType sizeType)
            {
                switch (sizeType)
                {
                    ...
                    case ControlSizeType.Final:
                        ...
                        var implSize = (ContainsItem ? ControlImplicitSize : null);                          // Implicitní velikost (pokud není určena jinak): Pokud dimenze má prvky, pak implicitní; pokud nemá prvky, pak 0
                        return ControlDesignSize ?? itemSize ?? implSize ?? 0;
                }
                return null;
            }
            /// <summary>
            ...
            private void _SetCurrentBegin(int? currentBegin)
            {
                if (currentBegin.HasValue)
                {
                    ...
                    // Pouze pokud tato dimenze reálně něco obsahuje (tedy její __CurrentEnd > __CurrentBegin), pak přidáme mezeru mezi dvěma sousedními dimenzemi 'DistanceAfterSize':
                    //  (pokud tato dimenze je zcela prázdná, pak __NextBegin == __CurrentEnd == __CurrentBegin)
                    if (__CurrentEnd.Value > __CurrentBegin.Value) position += DistanceAfterSize;
    ...
    internal interface IFlowLayoutItem
    {
        ...
        string Name { get; }
        /// <summary>
        /// Aktuální prvek je Container = má nějaké Child prvky. Pro takový prvek se volá metoda <see cref="InnerBoundsProcessed()"/>.
        /// </summary>
        ...
        /// <summary>
        /// Obsahuje prostory, které v tomto Itemu obsazují Labely
        /// </summary>
        DfLabelUsedAreaType LabelUsedAreas { get; }
        ...
        // Control a Labely mají svoje data pohromadě:
        /// <summary>
        /// Info o controlu
        /// </summary>
        ...
        /// <summary>
        /// Volá se pouze pro Containery (jejichž je <see cref="IsContainer"/> true), volá se po vložení souřadnic do Bounds do <see cref="ControlInfo"/> a do jednotlivých Labelů.
        /// V tuto chvíli je platný obsah <see cref="CellMatrix"/>.
        /// Container může na základě těchto hodnot modifikovat souřadnice svých Child prvků.
        /// </summary>
        void InnerBoundsProcessed();
        /// <summary>
        /// Resetuje výsledné hodnoty FlowLayoutu. Volá se na začátku finalizace layoutu jako první metoda.
        /// Neresetuje designové hodnoty. Neresetuje umístění do Matrixu. Resetuje to co souvisí s pixely.
        /// </summary>
    ...
    internal class ControlInfo
    {
        ...
        public override string ToString()
        {
            if (!Exists) return "(not exists)";
            string text = Text;
            if (Bounds != null) text += "; Bounds: " + Bounds.ToString();
            return text;
        }
        /// <summary>
        /// Existuje Control
        ...
        /// </summary>
        public DataControlType ControlType { get; set; }
        /// <summary>
        ...
        /// </summary>
        public DfFontInfo Font { get; set; }
        /// <summary>
        ...
        /// Šířka daná designem
        /// </summary>
        public Int32P? DesignWidth { get; set; }
        /// <summary>
        /// Výška daná designem
        /// </summary>
        public Int32P? DesignHeight { get; set; }

        /// <summary>
        /// Designová šířka v pixelech nebo null
        /// </summary>
        public int? DesignWidthPixel { get { return DesignWidth?.NumberPixel; } }
        /// <summary>
        /// Designová šířka v procentech nebo null
        /// </summary>
        public int? DesignWidthPercent { get { return DesignWidth?.NumberPercent; } }
        /// <summary>
        /// Designová výška v pixelech nebo null
        /// </summary>
        public int? DesignHeightPixel { get { return DesignHeight?.NumberPixel; } }
        /// <summary>
        /// Designová výška v procentech nebo null
        /// </summary>
        public int? DesignHeightPercent { get { return DesignHeight?.NumberPercent; } }
        ...
        /// </summary>
        public int? ImplicitMinimalWidth { get; set; }
        /// <summary>
        ...
        /// </summary>
        public int? ImplicitOptimalWidth { get; set; }
        /// <summary>
        ...
        /// </summary>
        public int? ImplicitMinimalHeight { get; set; }
        /// <summary>
        ...
        /// </summary>
        public int? ImplicitOptimalHeight { get; set; }
        ...
        /// <summary>
        /// Vypočte a vrátí souřadnice pro this Control, na základě svých dat, dodaného vyhrazeného prostoru a dodaného zarovnání obsahu.
        /// Control sám ví, jaký by měl mít rozměr (DesignPixel, DesignPercent, Optimal) pro osy Width i Height.
        /// </summary>
        /// <param name="totalBounds">Prostor, v němž se Control má umístit</param>
        /// <param name="designHPosition">Zarovnání horizontální (Left / Center / Right)</param>
        /// <param name="designVPosition">Zarovnání vertikální (Top / Center / Bottom)</param>
        /// <returns></returns>
        internal ControlBounds GetControlBounds(ControlBounds totalBounds, HPositionType? designHPosition, VPositionType? designVPosition)
        {
            ...
            // ControlWidth:
            int bw = totalBounds.Width;
            int cw = bw;
            if (this.DesignWidthPixel.HasValue) cw = this.DesignWidthPixel.Value;                            // Designer zadal do frm.xml hodnotu 'Width="85"'
            else if (this.DesignWidthPercent.HasValue) cw = (bw * this.DesignWidthPercent.Value / 100);      // Designer zadal do frm.xml hodnotu 'Width="50%"'
            else if (this.ImplicitOptimalWidth.HasValue) cw = this.ImplicitOptimalWidth.Value;               // Kód určil optimální šířku, které se držíme
            cw = (cw < 0 ? 0 : (cw > bw ? bw : cw));                                                         // 'cw' zarovnat do mezí 0 až 'bw'

            // ControlHeight:
            int bh = totalBounds.Height;
            int ch = bh;
            if (this.DesignHeightPixel.HasValue) ch = this.DesignHeightPixel.Value;                          // Designer zadal do frm.xml hodnotu 'Height="50"'
            else if (this.DesignHeightPercent.HasValue) ch = (bh * this.DesignHeightPercent.Value / 100);    // Designer zadal do frm.xml hodnotu 'Height="100%"'
            else if (this.ImplicitOptimalHeight.HasValue) ch = this.ImplicitOptimalHeight.Value;             // Kód určil optimální výšku, které se držíme
            ch = (ch < 0 ? 0 : (ch > bh ? bh : ch));                                                         // 'ch' zarovnat do mezí 0 až 'bh'

            // Bude nějaké zarovnání controlu (pokud má control šířku menší je šířka prostoru, a podobně pro výšku)?
            int cl = totalBounds.Left;
            if (cw < bw)
            {   // Vodorovně na střed / doprava?
                var hPos = designHPosition ?? HPositionType.Left;
                cl = getControlBegin(cl, bw, cw, hPos == HPositionType.Center, hPos == HPositionType.Right);
            }
            int ct = totalBounds.Top;
            if (ch < bh)
            {   // Svisle na střed / dolů?
                var vPos = designVPosition ?? VPositionType.Top;
                ct = getControlBegin(ct, bh, ch, vPos == VPositionType.Center, vPos == VPositionType.Bottom);
            }

            return new ControlBounds(cl, ct, cw, ch);
    ...
    internal class CellMatrixInfo
    {
        ...
        public string Text
        {
            get
            {
                return $"Left: {CellLeft}; Right: {CellRight}; Top: {CellTop}; Bottom: {CellBottom}";
            }
        }
        /// <summary>
        /// Debug text plný
        ...
        public string TextFull
        {
            ...
            {
                return $"Left: {CellLeft}; Right: {CellRight}; Top: {CellTop}; Bottom: {CellBottom}.   " +
                    $"LeftLabelWidth: {LeftLabelWidth}; ControlLeft: {ControlLeft}; ControlWidth: {ControlWidth}; RightLabelLeft: {RightLabelLeft}; RightLabelWidth: {RightLabelWidth}.   " +
                    $"TopLabelHeight: {TopLabelHeight}; ControlTop: {ControlTop}; ControlHeight: {ControlHeight}; BottomLabelTop: {BottomLabelTop}; BottomLabelHeight: {BottomLabelHeight}.";
            }
        }
        /// <summary>
        /// Pole souřadnic. <br/>
        ...
        /// </summary>
        public int MarginControlLeft { get { return ControlLeft - LeftLabelRight; } }
        /// <summary>
        /// Okraj nahoře od controlu, pod TopLabel
        /// </summary>
        public int MarginControlTop { get { return ControlTop - TopLabelBottom; } }
        /// <summary>
        /// Okraj vpravo od controlu, před RightLabel
        /// </summary>
        public int MarginControlRight { get { return RightLabelLeft - ControlRight; } }
        /// <summary>
        /// Okraj dole od controlu, před BottomLabel
        /// </summary>
        public int MarginControlBottom { get { return BottomLabelTop - ControlBottom; } }

        /// <summary>
        /// Výška prvního řádku v Controlu
        /// </summary>
        public int ControlFirstRowHeight { get { return ControlFirstBottom - ControlTop; } }
        /// <summary>
        /// Šířka prvního sloupce v Controlu
        /// </summary>
        public int ControlFirstColumnWidth { get { return ControlFirstRight - ControlLeft; } }

        /// <summary>
        /// Obsahuje prostory, které v tomto <see cref="CellMatrixInfo"/> obsazují Labely na konkrétní straně buňky (je tam pro Label vyhrazen prostor větší než 0)
        /// </summary>
        internal DfLabelUsedAreaType LabelUsedAreas
        {
            get
            {
                return getOneUsedLabelArea(this.LeftLabelLeft, this.LeftLabelRight, DfLabelUsedAreaType.Left) |
                       getOneUsedLabelArea(this.TopLabelTop, this.TopLabelBottom, DfLabelUsedAreaType.Top) |
                       getOneUsedLabelArea(this.RightLabelLeft, this.RightLabelRight, DfLabelUsedAreaType.Right) |
                       getOneUsedLabelArea(this.BottomLabelTop, this.BottomLabelBottom, DfLabelUsedAreaType.Bottom);

                // Pokud daný LabelInfo je zadán a existuje, pak vrátí daný typ UsedArea
                DfLabelUsedAreaType getOneUsedLabelArea(int labelBegin, int labelEnd, DfLabelUsedAreaType area)
                {
                    return (labelEnd > labelBegin) ? area : DfLabelUsedAreaType.None;
                }
            }
        }
        ...
        /// <param name="topLabelOffsetX">Offset labelu na pozici Top; pokud bude záporný (Top label je předsazen doleva před Control), pak souřadnice Controlu bude odsunuta doprava o toto předsazení</param>
        /// <param name="bottomLabelOffsetX">Offset labelu na pozici Bottom; pokud bude záporný (Bottom label je předsazen doleva před Control), pak souřadnice Controlu bude odsunuta doprava o toto předsazení</param>
        /// <returns></returns>
        internal ControlBounds GetControlBounds(ExpandControlType? expandControl = null, DfLabelUsedAreaType? usedAreas = null, int? topLabelOffsetX = null, int? bottomLabelOffsetX = null)
        {
            var expand = expandControl ?? ExpandControlType.None;
            var areas = usedAreas ?? DfLabelUsedAreaType.None;

            // Kam reálně budeme expandovat?   Kam chceme, a kde dosud nic není:
            var expL = expand.HasFlag(ExpandControlType.Left) && !areas.HasFlag(DfLabelUsedAreaType.Left);
            var expT = expand.HasFlag(ExpandControlType.Top) && !areas.HasFlag(DfLabelUsedAreaType.Top);
            var expR = expand.HasFlag(ExpandControlType.Right) && !areas.HasFlag(DfLabelUsedAreaType.Right);
            var expB = expand.HasFlag(ExpandControlType.Bottom) && !areas.HasFlag(DfLabelUsedAreaType.Bottom);

            int l = expL ? correctExpandedControlLeftByOffset(this.CellLeft) : this.ControlLeft;
            ...
            // Koriguje souřadnici Control.Left v situaci, kdy je Expanded doleva, tedy Control začíná přímo na souřadnici Cell.Left, 
            ...
            {
                // Label na pozici Left není přítomen, proto mohu (když chci) Control expandovat doleva (nic tam nepřekáží).
                // Pokud ale existuje Label na pozici Top (nebo Bottom) a v dané pozici je definován záporný offset 'topLabelOffsetX' (nebo 'bottomLabelOffsetX'),
                //  pak zajistím, že daný Label bude vlevo na souřadnici 0, a Control bude odsazen doprava o odpovídající Shift:
                int shiftT = ((areas.HasFlag(DfLabelUsedAreaType.Top) && topLabelOffsetX.HasValue && topLabelOffsetX.Value < 0) ? -topLabelOffsetX.Value : 0);
                if (shiftT < 0) shiftT = 0;
                int shiftB = ((areas.HasFlag(DfLabelUsedAreaType.Bottom) && bottomLabelOffsetX.HasValue && bottomLabelOffsetX.Value < 0) ? -bottomLabelOffsetX.Value : 0);
                if (shiftB < 0) shiftB = 0;

                // Beru větší Shift z Top / Bottom:
    ...
    internal class FlowGuideLine
    {
        ...
        /// <summary>
        /// Komparátor podle LineType, Axis, Position
        /// </summary>
        /// <param name="a"></param>
        /// <param name="b"></param>
        /// <returns></returns>
        internal static int CompareByLineType(FlowGuideLine a, FlowGuideLine b)
        {
            ...
            int ad = (a.Axis == AxisType.X ? 0 : 1);
            int bd = (b.Axis == AxisType.X ? 0 : 1);
            cmp = ad.CompareTo(bd);
            if (cmp != 0) return cmp;
        ...
        /// <summary>
        /// Komparátor podle Axis, Position
        /// </summary>
        /// <param name="a"></param>
        /// <param name="b"></param>
        /// <returns></returns>
        internal static int CompareByPosition(FlowGuideLine a, FlowGuideLine b)
        {
            int cmp;
    ...
    /// </summary>
    [Flags]
    internal enum DfLabelUsedAreaType
    {
        None = 0,
        Left = 0x01,
        Top = 0x02,
    ...
    internal class DfTemplateLayoutArgs : DfProcessArgs
    {
        ...
        /// <summary>
        /// Mají se ukládat Debug obrázky? Pak budou jejich jména uložena v <see cref="ImageFiles"/>
        /// </summary>
        public DfTemplateLayoutDebugImageMode CreateDebugImages { get; set; }
        /// <summary>
        /// Do Debug obrázku vykreslovat i vodící linky a další prvky?
        /// </summary>
        public DfTemplateLayoutDebugInfoLevelType DebugImagesInfoLevel { get; set; }
        /// <summary>
        /// Podadresář pro DebugImages v rámci Windows Temp adresáře
        /// </summary>
        public string DebugImagePath { get; set; }
        /// <summary>
        /// Objekty Image obsahující Název obrázku a Obrázek, pokud <see cref="CreateDebugImages"/> je <see cref="DfTemplateLayoutDebugImageMode.Image"/>
        /// </summary>
        public List<Tuple<string, System.Drawing.Image>> Images { get; set; }
        /// <summary>
        /// Plné názvy souborů s DebugImage soubory, pokud <see cref="CreateDebugImages"/> je <see cref="DfTemplateLayoutDebugImageMode.File"/>
        /// </summary>
        public List<string> ImageFiles { get; set; }
    }
    /// <summary>
    /// V jakém režimu generovat Debug image
    /// </summary>
    internal enum DfTemplateLayoutDebugImageMode
    {
        /// <summary>
        /// Negenerovat
        /// </summary>
        None,
        /// <summary>
        /// Vytvořit Image a vložit je do <see cref="DfTemplateLayoutArgs.Images"/>
        /// </summary>
        Image,
        /// <summary>
        /// Vytvořit Image a uložit je do souboru do adresáře (WinTemp)/<see cref="DfTemplateLayoutArgs.DebugImagePath"/>, a výsledná jména souborů uložit do <see cref="DfTemplateLayoutArgs.ImageFiles"/>
        /// </summary>
        File
    }
    /// <summary>
    /// Jaké grafické informace vykreslovat do DebugImage
    /// </summary>
    [Flags]
    internal enum DfTemplateLayoutDebugInfoLevelType
    {
        /// <summary>
        /// Žádné debug informace = čistý obraz formuláře
        /// </summary>
        None = 0,
        /// <summary>
        /// Vykresli pozadí pod Labely
        /// </summary>
        AddLabelBackground = 0x0010,
        /// <summary>
        /// Vykresli okraje kolem labelů
        /// </summary>
        AddLabelBorder = 0x0020,
        /// <summary>
        /// Vykresli vodící linky
        /// </summary>
        AddGuideLines = 0x0100,

        /// <summary>
        /// Všechno
        /// </summary>
        All = AddLabelBackground | AddLabelBorder | AddGuideLines
