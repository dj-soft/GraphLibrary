...
namespace DjSoft.Tools.ProgramLauncher.Data
{
    ...
    public static class Win32
    {
        ...
        #endregion
        #region Zpětná konverze
        /// <summary>
        /// Vrátí název MSG pro dané číslo
        /// </summary>
        ...
        public static string GetMsgName(int msg)
        {
            if (__Messages is null)
                __Messages = _LoadMsgNames();
            if (__Messages.TryGetValue(msg, out var name))
                return name;
            return "0x" + msg.ToString("X8");
        }
        private static Dictionary<int, string> _LoadMsgNames()
        {
            var messages = new Dictionary<int, string>();
            var fields = typeof(Win32.WM).GetFields();
            foreach (var field in fields) 
            {
                if (field.MemberType == System.Reflection.MemberTypes.Field && field.IsLiteral && field.IsStatic)
                {
                    var value = field.GetValue(null);
                    if (value is Int32 number)
                    {
                        if (!messages.ContainsKey(number))
                            messages.Add(number, field.Name);
                    }
                }
            }
            return messages;
        }
        private static Dictionary<int, string> __Messages;
        #endregion
    }
}
